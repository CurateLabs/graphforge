...
fn validate_algorithm_run(row: &AlgorithmRun) -> Result<(), KnowledgeError> {
    ...
    if row.algorithm.is_empty()
        || row.algorithm.len() as u64 > gf_core::canonical::MAX_CANONICAL_TEXT_BYTES
    {
    ...
    if row.descriptor.is_empty()
        || row.descriptor.len() as u64 > gf_core::canonical::MAX_CANONICAL_BINARY_BYTES
    {
...
fn validate_algorithm_run_event(row: &AlgorithmRunEvent) -> Result<(), KnowledgeError> {
    ...
    if row.error_code.as_ref().is_some_and(|code| {
        code.is_empty()
            || code.len() as u64 > gf_core::canonical::MAX_CANONICAL_TEXT_BYTES
...
fn validate_claim(claim: &str) -> Result<(), KnowledgeError> {
    ...
    if claim.len() as u64 > gf_core::canonical::MAX_CANONICAL_TEXT_BYTES {
...
mod tests {
    ...
    fn invalid_claims_refs_and_ordinals_fail_structurally() {
        ...
        let oversized = "x".repeat(gf_core::canonical::MAX_CANONICAL_TEXT_BYTES as usize + 1);
        assert!(matches!(
