...
use gf_core::PropId;
use gf_ir::expr::{BinaryOpKind, IrExpr, IrLiteral, UnaryOpKind};
use gf_ir::{ExprArena, ExprId, VarId};
...
pub enum LoweringError {
    /// A [`FunctionCall`](gf_ir::expr::IrExpr::FunctionCall) name has no
...
impl<'a> ExprLowerer<'a> {
        ...
        match self.arena.get(id) {
            ...
            IrExpr::PropertyAccess { base, prop } => {
                    ...
                    && let Some(shape) = self.node_shapes.get(&v.0)
                {
                    ...
                    let is_topology = gf_storage::TOPOLOGY_NODES_SCHEMA
    ...
    fn sole_arg_is_null(&self, args: &[ExprId]) -> bool {
        matches!(args, [a] if matches!(self.arena.get(*a), IrExpr::Literal(gf_ir::IrLiteral::Null)))
    }
    ...
    fn path_node_hydration(&self) -> Option<PathNodeHydration> {
        ...
        let stems = gf_storage::list_property_stems(dir);
        let mut fields = vec![
        ...
        for stem in &stems {
            let table = gf_storage::PropertyTable::open_discovered(dir, stem);
...
/// preserving the lowercase dotted-property-column scheme (`gf-plan`).
...
///
/// The binder interns properties into the [`RuntimeCatalog`](gf_ir::RuntimeCatalog)
...
impl ScalarUDFImpl for CypherDurationBetween {
    ...
    fn return_type(&self, _arg_types: &[DataType]) -> datafusion::error::Result<DataType> {
        Ok(DataType::Struct(
            gf_storage::schemas::duration_struct_fields(),
        ))
    }
...
impl CypherInvariantQuantifier {
    fn new(kind: gf_ir::QuantifierKind, predicate: Option<bool>) -> Self {
        Self {
            kind,
            predicate,
            signature: Signature::any(1, Volatility::Immutable),
...
impl ScalarUDFImpl for CypherDateProject {
    ...
    fn return_type(&self, _arg_types: &[DataType]) -> datafusion::error::Result<DataType> {
        Ok(DataType::Struct(gf_storage::schemas::date_struct_fields()))
    }
...
fn date_fields() -> datafusion::arrow::datatypes::Fields {
    gf_storage::schemas::date_struct_fields()
}
...
fn localdatetime_fields() -> datafusion::arrow::datatypes::Fields {
    gf_storage::schemas::localdatetime_struct_fields()
}
...
fn time_fields() -> datafusion::arrow::datatypes::Fields {
    gf_storage::schemas::time_struct_fields()
}
...
fn datetime_fields() -> datafusion::arrow::datatypes::Fields {
    gf_storage::schemas::datetime_struct_fields()
}
...
fn path_node_labels_child(
    ...
    let exec_err = |m: String| DataFusionError::Execution(m);
    let node_batches = gf_storage::read_nodes(&h.dir).map_err(|e| exec_err(e.to_string()))?;
...
fn path_node_prop_children(
    ...
    for stem in &h.prop_stems {
        let bs = gf_storage::read_properties(&h.dir, stem).map_err(|e| exec_err(e.to_string()))?;
...
mod tests {
    ...
    fn quantifier_three_valued_reduce() {
        use datafusion::arrow::array::BooleanArray;
        use gf_ir::QuantifierKind::{All, Any, None, Single};
    ...
    fn invariant_quantifier_truth_matrix_preserves_cardinality_and_nulls() {
        use gf_ir::QuantifierKind::{All, Any, None as NoneQ, Single};
    ...
    fn invariant_quantifier_lowering_uses_cardinality_only_udf() {
        use gf_ir::QuantifierKind::None as NoneQ;
    ...
    fn boolean_and_numeric_operators_reject_known_bad_operands() {
        use gf_ir::expr::{BinaryOpKind as B, IrExpr, IrLiteral, UnaryOpKind as U};
        let vm = VarMap::new();
    ...
    fn nested_quantifiers_get_per_depth_elem_columns() {
        use gf_ir::QuantifierKind::{None as NoneQ, Single};
