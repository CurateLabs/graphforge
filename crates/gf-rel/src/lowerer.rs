...
//! | `Expand { variable-length }` | `VarLenExpandNode` (`gf-plan` Extension stub) |
//! | `Optional { child }` | `OptionalMatchNode` (`gf-plan` Extension stub) |
...
//! | `Unwind { list_expr, alias }` | `UnwindNode` (`gf-plan` Extension stub) |
//!
//! Graph-native operators (#578) lower to `gf-plan` logical stub nodes wrapped
...
use gf_core::{GfError, OntologyMode, TypeId};
use gf_ir::plan::PATTERN_COMPREHENSION_VALUE_ALIAS;
use gf_ir::{
...
use gf_storage::{EXPLORATORY_EDGE_SCHEMA, GraphCatalog, TOPOLOGY_NODES_SCHEMA, TYPED_EDGE_SCHEMA};
...
impl<'a> GraphPlanLowerer<'a> {
    ...
    fn build(
        ...
        if let Some(c) = catalog {
            for (id, name) in c.rel_names() {
                let plan_id = gf_ir::runtime_relation_type_id(gf_ir::RuntimeTypeId(*id));
    ...
    fn node_prop_cols(&self, ty: Option<TypeId>) -> Vec<String> {
        ...
        let prop_table = gf_storage::PropertyTable::open_discovered(dir, &stem);
        prop_table
            .schema_ref()
            .fields()
            .iter()
    ...
    //
    // The driver in gf-exec runs one read prefix and then applies the write
        ...
        match op {
            ...
            GraphOp::RelationshipUnique { edge, prior_edges } => {
                ...
                let mut predicates = prior_edges.iter().map(|prior| {
                    ...
                    let value = |alias: &str| {
                        if alias.ends_with(gf_plan::VAR_LEN_EDGE_LIST_FIELD) {
                            col(alias)
                        } else {
            ...
            // UNWIND — emit the gf-plan stub Extension node (physical execution
    ...
    fn lower_call_op(
        &self,
        procedure: &gf_ir::ProcedureDefinition,
        args: &[ExprId],
        yields: &[gf_ir::ProcedureYield],
        input: LogicalPlan,
        exprs: &ExprArena,
        var_map: &mut VarMap,
    ) -> Result<LogicalPlan, LoweringError> {
...
fn node_scan_source(dir: Option<&Path>) -> Arc<dyn datafusion::logical_expr::TableSource> {
    ...
    match dir {
        Some(d) => {
            ...
            provider_as_source(Arc::new(gf_storage::TopologyNodeTable::new(path)))
        }
...
/// every per-relation file via [`gf_storage::UnionEdgeTable`] (#823) — the same
...
fn edge_scan_source(
    ...
    match dir {
        ...
        {
            provider_as_source(Arc::new(gf_storage::UnionEdgeTable::open(d)))
        }
        Some(d) => provider_as_source(Arc::new(gf_storage::TypedEdgeTable::open(d, stem))),
...
fn lower_var_len_expand(
    ...
    let prop_fields: Vec<datafusion::arrow::datatypes::Field> = if rel_name == "*" {
        ...
        for stem in gf_storage::list_edge_property_stems(dir_path) {
            let prop_table = gf_storage::EdgePropertyTable::open_discovered(dir_path, &stem);
        ...
        let prop_table = gf_storage::EdgePropertyTable::open_discovered(dir_path, &rel_name);
        prop_table
            .schema_ref()
            .fields()
            .iter()
    ...
    // so gf-plan need not depend on gf-storage; the edge-list field type is the
    // shared single source of truth in gf-plan.
    ...
    let node = VarLenExpandNode::new(
        ...
        mode,
        dst_fields,
        gf_plan::var_len_edge_list_field(&prop_fields),
    );
    ...
    var_map.insert(
        edge,
        format!("var_{}.{}", edge.0, gf_plan::VAR_LEN_EDGE_LIST_FIELD),
    );
    ...
    if let Some(bound_edge_list) = bound_edge_list {
        let produced = format!("var_{}.{}", edge.0, gf_plan::VAR_LEN_EDGE_LIST_FIELD);
        base = LogicalPlanBuilder::from(base)
    ...
    let plan = infer_rules
        ...
        .fold(base, |acc, (rule_id, conf_model)| {
            LogicalPlan::Extension(Extension {
                node: Arc::new(gf_plan::OntologyInferNode::new(
                    Arc::new(acc),
                    rel_for_infer.clone(),
                    rule_id,
                    conf_model,
...
fn lower_expand(
    ...
    // the gf-plan Extension node whose physical execution (M13) performs an
    // iterative BFS over the edge table.
...
/// Emit the provider-backed [`gf_plan::ExpandNode`] for a project-backed fixed
/// hop (#1248).
///
...
fn try_lower_provider_expand(
    ...
    let mut stems = if rel_name == "*" {
        gf_storage::list_edge_property_stems(dir_path)
    } else {
        vec![rel_name.clone()]
    };
    stems.sort();
    ...
    let node = gf_plan::ExpandNode::new(
        Arc::new(input.clone()),
        rel_name,
        src.0,
        traversal_dst.0,
...
fn join_edge_properties(
    ...
    let mut push_source = |stem: String| {
        let table = gf_storage::EdgePropertyTable::open_discovered(dir, &stem);
    ...
    if let Some(rel_ty) = rel_ty {
        ...
        for stem in gf_storage::list_edge_property_stems(dir) {
            push_source(stem);
        }
    }
...
mod tests {
    ...
    use gf_core::TypeId;
    use gf_ir::expr::{IrExpr, IrLiteral};
    use gf_ir::{Direction, ExprArena, GraphPlan, VarId};
    ...
    fn make_catalog_and_lowerer() -> (
        tempfile::TempDir,
        gf_storage::GraphCatalog,
        gf_ir::RuntimeCatalog,
    ) {
        let dir = tempfile::TempDir::new().unwrap();
        let rc = gf_ir::RuntimeCatalog::new();
        let catalog = gf_storage::GraphCatalog::open(dir.path(), None, &rc).unwrap();
        (dir, catalog, rc)
    }
    ...
    fn create_plan_with_props() -> GraphPlan {
        use gf_ir::{CreateNodeSpec, CreatePattern};
        let mut builder = GraphPlan::builder("openCypher");
    ...
    fn create_non_literal_property_lowers_as_computed() {
        ...
        let create = ext
            .node
            .as_any()
            .downcast_ref::<gf_plan::GraphCreateNode>()
    ...
    fn writes_lowerer<'a>(
        catalog: &'a gf_storage::GraphCatalog,
        dir: &'a std::path::Path,
    ) -> GraphPlanLowerer<'a> {
    ...
    fn set_runtime_expr_value_lowers_against_input_schema() {
        ...
        let sum = builder.push_expr(IrExpr::BinaryOp {
            op: gf_ir::expr::BinaryOpKind::Add,
            left: age,
            right: one,
        });
        let plan = builder
    ...
    fn typed_single_hop_fixture(
        dir: Direction,
    ) -> (tempfile::TempDir, gf_storage::GraphCatalog, GraphPlan) {
        let tmp = tempfile::TempDir::new().unwrap();
        let mut rc = gf_ir::RuntimeCatalog::new();
        let rel = gf_ir::runtime_relation_type_id(rc.intern_relation_type("KNOWS"));
        let catalog = gf_storage::GraphCatalog::open(tmp.path(), None, &rc).unwrap();
        let plan = GraphPlan::builder("openCypher")
            .push_op(GraphOp::NodeScan {
                var: VarId(0),
                ty: None,
    ...
    fn project_backed_single_hop_emits_expand_extension_node() {
        ...
        let node = ext
            .node
            .as_any()
            .downcast_ref::<gf_plan::ExpandNode>()
            .expect("ExpandNode");
