...
///
/// Using a type alias means all downstream code (including `gf-exec`) works
...
/// [`GraphCatalog`](gf_storage::GraphCatalog) so property accesses (e.g.
...
pub fn explain_logical_with_catalog(
    plan: &GraphPlan,
    catalog: Option<&gf_storage::GraphCatalog>,
    ontology: Option<&OntologyHandle>,
) -> Result<String, GfError> {
