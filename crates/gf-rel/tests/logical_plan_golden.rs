...
//! harness in `crates/gf-ir/tests/golden.rs`.
//!
//! **Updating fixtures:**
//! ```
//! INSTA_UPDATE=always cargo test -p gf-rel --test logical_plan_golden
//! ```
//!
//! ## Why these queries
//!
...
//!
//! Snapshots are rendered with [`gf_rel::explain_logical_with`], which runs the
...
// ---------------------------------------------------------------------------
// Setup helpers (mirrors crates/gf-ir/tests/golden.rs)
// ---------------------------------------------------------------------------
...
fn render(query: &str) -> String {
    ...
    gf_rel::explain_logical_with(&plan, Some(&handle))
...
fn render_lowered(query: &str) -> String {
    ...
    let lowered = gf_rel::GraphPlanLowerer::new_with_dir(
        None,
        Some(&handle),
        dir.path(),
...
fn render_property_read(query: &str) -> String {
    ...
    use gf_core::TypeId;
    use gf_core::uuid::new_v7;
    use gf_ir::{Binder, IrLiteral, RuntimeCatalog};
    use gf_storage::{GraphCatalog, GraphWriter};

    let rc = Arc::new(Mutex::new(RuntimeCatalog::new()));
    let binder = Binder::new(None, rc.clone(), OntologyMode::Exploratory);
    let ast = gf_cypher::parse(query).unwrap_or_else(|e| panic!("parse {query:?}: {e}"));
    let plan = binder
        .bind(&ast)
...
// ---------------------------------------------------------------------------
// Golden scenarios — graph-native Extension stubs (gf-plan, physical exec M13)
// ---------------------------------------------------------------------------
