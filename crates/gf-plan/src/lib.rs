...
/// holds (the gf-api Shaper passes the whole column through untouched).
///
...
/// sequence, so the physical layer ([`VarLenExpandExec`](../gf_exec) in M13)
...
/// (`var_<dst>`-qualified [`TOPOLOGY_NODES_SCHEMA`](../gf_storage)), then a
...
impl VarLenExpandNode {
    ...
    /// need not depend on `gf-storage`.  They are qualified `var_<dst_var>` and
...
/// read target is available. The physical `ExpandExec` (gf-exec) probes the
...
impl ExpandNode {
    ...
    /// columns. Passed in so this crate need not depend on gf-storage.
    #[allow(clippy::too_many_arguments)]
    #[must_use]
    pub fn new(
        input: Arc<LogicalPlan>,
...
///
/// The physical layer ([`OptionalMatchExec`](../gf_exec) in M13) left-joins the
...
impl OptionalMatchNode {
    ...
    /// `gf-rel`/`gf-storage` dependency. The node appends the corresponding
...
///
/// Built by the relational lowering layer from a `gf_ir::CreateNodeSpec`:
...
///
/// The physical layer ([`UnwindExec`](../gf_exec) in M13) evaluates `list_expr`
...
impl UnwindNode {
    ...
    /// `gf-rel` dependency). It is appended to the input schema qualified by
