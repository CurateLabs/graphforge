...
use gf_ontology::OntologyLoader;
...
fn recorded_at(result: &gf_api::ExecutionResult) -> i64 {
    result.batches[0]
...
fn main() {
    ...
    let compiled = gf_ontology::OntologyCompiler::compile(&ontology).unwrap();
    let ontology_fingerprint = gf_ontology::OntologyHandle::new(compiled)
        .checksum()
        .to_owned();
    ...
    for uuid in person_ids {
        let rows = reopened.list_assertions(gf_api::ListAssertionsRequest {
