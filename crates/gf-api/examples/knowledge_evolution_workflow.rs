...
fn main() {
    ...
    let neutral_paths = graph
        .paths(
            Some(&gf_api::NodeSelector::Handle(alpha.clone())),
            Some(&gf_api::NodeSelector::Handle(source.clone())),
            PathsOptions {
    ...
    let valid = graph
        .apply_valid_time(gf_api::ApplyValidTimeRequest {
            transaction_cutoff_micros: cutoff_unselected,
    ...
    let after_paths = graph
        .paths(
            Some(&gf_api::NodeSelector::Handle(alpha)),
            Some(&gf_api::NodeSelector::Handle(source)),
            PathsOptions {
                by: PathAlgorithm::Bfs,
                via: Some("SUPPORTED_BY".into()),
                directed: true,
