...
impl GraphForge {
    ...
    fn resolve_property_match(
        ...
        for stem in gf_storage::list_property_stems(&self.dir) {
            for (bytes, properties) in gf_storage::read_node_property_rows(&self.dir, &stem)? {
                scanned += 1;
    ...
    fn node_uuids(&self, label_id: Option<u32>) -> Result<HashSet<Uuid>, GfError> {
        ...
        let mut scanned = 0usize;
        for batch in gf_storage::read_nodes(&self.dir)
...
mod tests {
    ...
    use gf_core::PathsOptions;

    fn first_uuid(graph: &GraphForge) -> Uuid {
        let batches = gf_storage::read_nodes(&graph.dir).unwrap();
    ...
    fn malformed_missing_ambiguous_and_cross_graph_selectors_are_rejected() {
        ...
        gf_storage::set_node_properties(&other.dir, "Person", &updates).unwrap();
