//! `gf-api` orchestration between neutral graph receipts and `gf-provenance`.
...
use gf_exec::{MutationKind, MutationReceipt, MutationSubject, MutationSubjectKind};
use gf_provenance::{
    EventKind, LineageRecord, LineageRole, ProvenanceEvent, ProvenanceLedger, SubjectKind,
    schema_registry,
};
use gf_storage::{ProjectParticipant, ProjectParticipantEncoding, ResolvedProjectGeneration};
...
fn participant(
    registry: &gf_provenance::SchemaRegistryEntry,
    batch: &RecordBatch,
...
fn execution_result(batch: RecordBatch) -> gf_exec::ExecutionResult {
...
mod tests {
    ...
    use gf_exec::{MutationEffect, MutationSubject};
    use std::collections::HashMap;
    ...
    fn enabled_project_commits_graph_and_provenance_in_one_generation() {
        ...
        let generation = gf_storage::resolve_project_generation(root.path()).unwrap();
        assert!(
            generation
        ...
        assert_eq!(
            reopened
                .resolve_node_selector(&gf_core::NodeSelector::Uuid(node.uuid))
                .unwrap(),
            node.uuid
        );
    }
    ...
    fn supported_mutation_matrix_records_closed_kinds_and_explicit_noops() {
        ...
        let before_noop =
            read_ledger(&gf_storage::resolve_project_generation(root.path()).unwrap()).unwrap();
        graph
