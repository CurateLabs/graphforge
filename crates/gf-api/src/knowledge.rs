//! `gf-api` orchestration for immutable UUID-referenced assertions.
...
impl GraphForge {
    ...
    pub fn create_assertion(
        ...
        let receipt = match gf_storage::stage_project_generation(root, &publication)? {
        ...
        *self
        ...
        let committed = gf_storage::resolve_project_generation(root)?;
    ...
    pub fn list_assertions(
        ...
        let rows = selected[start..end]
            ...
            .collect::<Vec<_>>();
        let batch = concat_or_empty(&rows, &gf_knowledge::ASSERTION_SCHEMA)?;
        let next =
    ...
    pub fn assertion_graph_refs(
        ...
        let rows = selected[start..end]
            ...
            .map(|index| source.slice(*index, 1))
            .collect::<Vec<_>>();
        let batch = concat_or_empty(&rows, &gf_knowledge::ASSERTION_GRAPH_REF_SCHEMA)?;
        let next =
            (end < selected.len()).then(|| PageToken::new(generation.generation_uuid(), end));
        Ok(assertion_result(with_next_token(&batch, next.as_ref())?))
    }
    ...
    pub fn list_confidence_assessments(
        ...
        let rows = selected[start..end]
            ...
            .map(|index| source.slice(*index, 1))
            .collect::<Vec<_>>();
        let batch = concat_or_empty(&rows, &gf_knowledge::CONFIDENCE_ASSESSMENT_SCHEMA)?;
        let next =
            (end < selected.len()).then(|| PageToken::new(generation.generation_uuid(), end));
        Ok(assertion_result(with_next_token(&batch, next.as_ref())?))
    }
    ...
    pub fn confidence_inputs(
        ...
        let rows = selected[start..end]
            ...
            .map(|index| source.slice(*index, 1))
            .collect::<Vec<_>>();
        let batch = concat_or_empty(&rows, &gf_knowledge::CONFIDENCE_INPUT_SCHEMA)?;
        let next =
            (end < selected.len()).then(|| PageToken::new(generation.generation_uuid(), end));
        Ok(assertion_result(with_next_token(&batch, next.as_ref())?))
    }
    ...
    pub fn list_evidence_links(
        ...
        let rows = selected[start..end]
            ...
            .map(|index| source.slice(*index, 1))
            .collect::<Vec<_>>();
        let batch = concat_or_empty(&rows, &gf_knowledge::EVIDENCE_LINK_SCHEMA)?;
        let next =
            (end < selected.len()).then(|| PageToken::new(generation.generation_uuid(), end));
        Ok(assertion_result(with_next_token(&batch, next.as_ref())?))
    }
    ...
    pub fn list_reasoning(
        ...
        let rows = selected[start..end]
            ...
            .map(|index| source.slice(*index, 1))
            .collect::<Vec<_>>();
        let batch = concat_or_empty(&rows, &gf_knowledge::REASONING_SCHEMA)?;
        let next =
            (end < selected.len()).then(|| PageToken::new(generation.generation_uuid(), end));
        Ok(assertion_result(with_next_token(&batch, next.as_ref())?))
    }
...
fn publish_confidence(
    ...
    *graph
        ...
        .lock()
        .expect("generation UUID lock poisoned") = receipt.generation_uuid;
    let ledger = read_confidence_ledger(&gf_storage::resolve_project_generation(root)?)?;
    let index = ledger
        .assessments
        .iter()
        .position(|row| row.confidence_uuid == request.confidence_uuid)
...
fn publish_evidence(
    ...
    *graph
        ...
        .lock()
        .expect("generation UUID lock poisoned") = receipt.generation_uuid;
    let ledger = read_evidence_ledger(&gf_storage::resolve_project_generation(root)?)?;
    let index = ledger
        .links
        .iter()
        .position(|row| row.evidence_uuid == request.evidence_uuid)
...
fn publish_assertion_evidence(
    ...
    *graph
        ...
        .lock()
        .expect("generation UUID lock poisoned") = receipt.generation_uuid;
    let ledger = read_ledger(&gf_storage::resolve_project_generation(root)?)?;
    let index = ledger
        .assertions
        .iter()
        .position(|row| row.assertion_uuid == request.assertion.assertion_uuid)
...
pub(crate) fn participant(
    registry: &gf_knowledge::SchemaRegistryEntry,
    batch: &RecordBatch,
...
fn match_requested_node_uuids(
    ...
    for batch in
        gf_storage::read_nodes(&graph.dir).map_err(|error| GfError::Storage(error.to_string()))?
    {
...
pub(crate) fn assertion_result(batch: RecordBatch) -> gf_exec::ExecutionResult {
    let rows = u64::try_from(batch.num_rows()).unwrap_or(u64::MAX);
    gf_exec::ExecutionResult {
        schema: batch.schema(),
        batches: vec![batch],
...
pub(crate) fn knowledge_error(error: gf_knowledge::KnowledgeError) -> GfError {
    let message = error.to_string();
    match error {
        gf_knowledge::KnowledgeError::Conflict(_)
        | gf_knowledge::KnowledgeError::TransactionConflict(_) => GfError::Project {
            code: ProjectErrorCode::TransactionConflict,
            message,
        },
        gf_knowledge::KnowledgeError::Limit { .. } => GfError::Api {
            code: ApiErrorCode::ResourceLimit,
            message,
        },
        gf_knowledge::KnowledgeError::Dangling(_) => GfError::Api {
            code: ApiErrorCode::NotFound,
            message,
        },
        gf_knowledge::KnowledgeError::Invalid { .. }
        | gf_knowledge::KnowledgeError::Duplicate(_)
        | gf_knowledge::KnowledgeError::Canonical(_) => GfError::Validation(message),
        gf_knowledge::KnowledgeError::Arrow(_) => GfError::Api {
            code: ApiErrorCode::SchemaMismatch,
            message,
        },
    }
...
pub(crate) fn provenance_error(error: gf_provenance::ProvenanceError) -> GfError {
    let message = error.to_string();
    match error {
        gf_provenance::ProvenanceError::Conflict(_) => GfError::Project {
            code: ProjectErrorCode::TransactionConflict,
            message,
        },
        gf_provenance::ProvenanceError::Limit { .. } => GfError::Api {
            code: ApiErrorCode::ResourceLimit,
            message,
        },
        gf_provenance::ProvenanceError::Invalid { .. }
        | gf_provenance::ProvenanceError::Duplicate(_)
        | gf_provenance::ProvenanceError::Dangling(_)
        | gf_provenance::ProvenanceError::Arrow(_) => GfError::Api {
            code: ApiErrorCode::SchemaMismatch,
            message,
        },
        gf_provenance::ProvenanceError::Canonical(_) => GfError::Validation(message),
    }
}
...
mod tests {
    ...
    fn reasoning_is_append_only_exact_idempotent_and_reopenable() {
        ...
        assert_eq!(
            ...
            Arc::clone(&gf_knowledge::REASONING_SCHEMA)
        );
        assert_eq!(
    ...
    fn assertion_publication_is_atomic_idempotent_and_reopenable() {
        ...
        let generation = gf_storage::resolve_project_generation(root.path())
            .unwrap()
            .generation_uuid();
        let replay = graph.create_assertion(request).unwrap();
        assert_eq!(replay.batches[0], created.batches[0]);
        assert_eq!(
            gf_storage::resolve_project_generation(root.path())
                .unwrap()
                .generation_uuid(),
            generation
        );
    ...
    fn invalid_graph_reference_and_conflicting_replay_publish_nothing() {
        ...
        let before = gf_storage::resolve_project_generation(root.path())
            .unwrap()
            .generation_uuid();
        let missing = graph
            .create_assertion(CreateAssertionRequest {
        ...
        let committed = gf_storage::resolve_project_generation(root.path())
            .unwrap()
            .generation_uuid();
