...
fn m21_inventory_json() -> Vec<u8> {
    let mut entries = gf_knowledge::schema_registry()
        .into_iter()
        .filter(|entry| matches!(entry.capability_id, "epistemic" | "valid_time"))
        .map(|entry| InventoryEntry {
            capability_id: entry.capability_id,
    ...
    json!([
        {
            ...
            "owner": "gf-storage",
            "format": String::from_utf8_lossy(gf_storage::PROJECT_FORMAT_BYTES),
            "version": 1,
            "fields": [],
        },
        {
    ...
    fn m20_schema_inventory_matches_checked_contract() {
        ...
        assert_eq!(
            ...
            "M20 schema registry drifted; review it, then run \
             UPDATE_M20_SCHEMA_INVENTORY=1 cargo test -p gf-api \
    ...
    fn m21_schema_inventory_matches_checked_contract() {
        ...
        assert_eq!(
            ...
            "M21 schema registry drifted; review it, then run \
             UPDATE_M21_SCHEMA_INVENTORY=1 cargo test -p gf-api \
