...
use gf_exec::{ExecutionResult, ExecutionStats};
use gf_knowledge::EPISTEMIC_CAPABILITY_VERSION;
use gf_storage::{
...
impl GraphForge {
    ...
    pub fn enable_capability(
        ...
        let generation_uuid = match gf_storage::stage_project_generation(root, &publication)? {
...
mod tests {
    ...
    fn future_capability_is_reported_without_opening_participants() {
        let root = tempfile::tempdir().unwrap();
        let initial = gf_storage::open_or_initialize_project(root.path()).unwrap();
        ...
        let ProjectStageOutcome::Staged(staged) =
            gf_storage::stage_project_generation(root.path(), &request).unwrap()
        else {
