...
use gf_ontology::PropertyValueType;
...
impl GraphForge {
    ...
    pub fn publish_bulk_nodes(
        ...
        if let Some(published) = gf_storage::published_project_transaction(root, operation_uuid.0)?
        {
        ...
        for row in &normalized.rows {
            let type_id = self
                ...
                .unwrap_or_else(|| gf_core::TypeId(next_catalog.intern_label(&row.label).0));
        ...
        let publication = (|| -> Result<(), super::GfError> {
            ...
            let receipt = gf_exec::MutationReceipt {
                effects: vec![gf_exec::MutationEffect {
                    kind: gf_exec::MutationKind::CreateNode,
                    inputs: Vec::new(),
                    outputs: normalized
                        .rows
                        .iter()
                        .map(|row| gf_exec::MutationSubject {
            ...
            let receipt = gf_exec::MutationReceipt {
                effects: vec![gf_exec::MutationEffect {
                    ...
                    inputs: normalized
                        ...
                        .iter()
                        .flat_map(|row| [row.source_uuid, row.target_uuid])
                        .map(|uuid| gf_exec::MutationSubject {
...
fn validate_ontology_field(
    ...
    let expected_arrow = gf_storage::property_type_to_arrow(expected);
...
fn existing_node_uuids(graph: &GraphForge) -> Result<BTreeSet<Uuid>, BulkValidationError> {
    ...
    for batch in gf_storage::read_nodes(&graph.dir).map_err(|error| {
        contract_error(
            BulkInputKind::Node,
            BulkValidationReason::ProjectState,
            &error.to_string(),
...
fn existing_edge_uuids(graph: &GraphForge) -> Result<BTreeSet<Uuid>, BulkValidationError> {
    let mut values = BTreeSet::new();
    for batch in gf_storage::read_edges(&graph.dir, "*", graph.ontology_mode).map_err(|error| {
        contract_error(
            BulkInputKind::Edge,
            BulkValidationReason::ProjectState,
            &error.to_string(),
...
fn register_existing_endpoints(
    ...
    for batch in gf_storage::read_nodes(dir).map_err(|error| {
