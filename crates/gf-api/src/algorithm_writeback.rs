...
use gf_core::algorithms::Algorithm;
use gf_ir::IrLiteral;
...
impl GraphForge {
    ...
    pub(crate) fn write_algorithm_property(
        ...
        let touched =
            gf_storage::stage_set_node_properties(&mut staged, &self.dir, stem, &updates)?;
        staged.commit()?;
        *catalog = next_catalog;
        ...
        let receipt = gf_exec::MutationReceipt {
            effects: vec![gf_exec::MutationEffect {
                kind: gf_exec::MutationKind::SetProperty,
                inputs: Vec::new(),
                outputs,
            }],
        };
...
fn reject_property_collision(
    ...
    let batches = gf_storage::read_properties(dir, stem)
...
mod tests {
    ...
    use gf_core::algorithms::{ClusterAlgorithm, RankAlgorithm};
    ...
    fn graph() -> (tempfile::TempDir, GraphForge, [u8; 16]) {
        ...
        let nodes = gf_storage::read_nodes(&graph.dir).unwrap();
    ...
    fn rank_and_cluster_writes_persist_without_topology_changes() {
        ...
        let props =
            gf_storage::read_entity_properties(&reopened.dir, "_untyped", &uuid, false).unwrap();
