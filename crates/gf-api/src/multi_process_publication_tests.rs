...
fn request(root: &Path, transaction_uuid: Uuid, generation_uuid: Uuid) -> ProjectGenerationRequest {
    let selected = gf_storage::resolve_project_generation(root).expect("phase=request resolve");
...
fn multi_process_writer_helper() {
    ...
    match mode.as_str() {
        "stage" => {
            let staged = match gf_storage::stage_project_generation(
                &root,
                &request(
                    &root,
        ...
        "contend" => {
            ...
            let error = match gf_storage::stage_project_generation(&root, &contender) {
...
fn killed_staged_child_releases_lock_and_recovers_without_partial_generation() {
    ...
    assert_eq!(
        gf_storage::resolve_project_generation(project.path())
    ...
    let recovered = gf_storage::recover_project_transactions(project.path())
    ...
    let reopened = gf_storage::resolve_project_generation(project.path())
    ...
    let recovered_again = gf_storage::recover_project_transactions(project.path())
    ...
    let staged = gf_storage::stage_project_generation(project.path(), &probe)
        .expect("phase=killed lock release probe");
    assert!(matches!(&staged, ProjectStageOutcome::Staged(_)));
    drop(staged);
    gf_storage::recover_project_transactions(project.path())
...
fn published_child_is_visible_only_to_fresh_current_reader() {
    ...
    seed(project.path());
    let pinned_generation = gf_storage::resolve_project_generation(project.path())
    ...
    let current = gf_storage::resolve_project_generation(project.path())
