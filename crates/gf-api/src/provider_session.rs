        ...
        match result {
            ...
            Err(
                ...
                | ProviderEmbeddingExecutionError::Publication(
                    gf_search::ProviderPublicationError::Artifact(SearchArtifactError::Cancelled),
                ),
            ) => EmbeddingRefreshCompletion::Cancelled,
            Err(
                ProviderEmbeddingExecutionError::Plan(ProviderEmbeddingPlanError::Provider(error))
                | ProviderEmbeddingExecutionError::Publication(
                    gf_search::ProviderPublicationError::Provider(error),
                ),
...
impl OpenRouterProviderSession {
    ...
    pub fn publish_embeddings(
        ...
        let mut estimate_cost = move |shape: gf_search::ProviderBatchShape| {
            ...
            OpenRouterHttpTransport::new(&self.config.origin, self.config.transport_timeout)
                .map_err(|_| {
                    gf_search::ProviderError::new(
                        &self.contract,
                        gf_search::ProviderFailureClass::InvalidRequest,
                    )
                })?;
        OpenRouterOwnedAdapter::new(
            self.contract.clone(),
...
fn estimate_cost(
    ...
    input_tokens.checked_mul(rate).ok_or_else(|| {
        gf_search::ProviderError::new(contract, gf_search::ProviderFailureClass::ResourceExhausted)
    })
}
...
#[cfg(test)]
mod tests {
    use gf_search::{ProviderCapability, ProviderFailureClass};
