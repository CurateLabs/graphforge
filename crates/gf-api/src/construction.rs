...
impl GraphForge {
    ...
    pub fn add_node(
        ...
        let strict_owner = match (self.ontology_mode, self.ontology.as_ref()) {
            (gf_core::OntologyMode::Strict, Some(ontology)) => ontology
    ...
    pub fn add_edge(
        ...
        let src_uuid = self.resolve_node_selector(&gf_core::NodeSelector::Handle(src.clone()))?;
        let dst_uuid = self.resolve_node_selector(&gf_core::NodeSelector::Handle(dst.clone()))?;
        ...
        let mut writer = gf_storage::GraphWriter::open_at(&self.dir, self.ontology_mode, now)?;
        ...
        writer.flush()?;
        let receipt = gf_exec::MutationReceipt {
            effects: vec![gf_exec::MutationEffect {
                kind: gf_exec::MutationKind::CreateEdge,
                inputs: vec![
                    gf_exec::MutationSubject {
                        uuid: src_uuid.into_bytes(),
                        kind: gf_exec::MutationSubjectKind::Node,
                    },
                    gf_exec::MutationSubject {
                ...
                outputs: vec![gf_exec::MutationSubject {
                    uuid: edge_uuid.into_bytes(),
                    kind: gf_exec::MutationSubjectKind::Edge,
                }],
            }],
        };
...
fn created_uuid(result: &gf_exec::ExecutionResult) -> Result<Uuid, GfError> {
...
mod tests {
    ...
    fn in_memory_node_is_readable_and_owned_by_its_graph() {
        ...
        assert_eq!(
            graph
                .resolve_node_selector(&gf_core::NodeSelector::Handle(handle.clone()))
                .unwrap(),
            handle.uuid
        );
        let result = graph
        ...
        assert!(
            other
                .resolve_node_selector(&gf_core::NodeSelector::Handle(handle))
                .is_err()
        );
    }
    ...
    fn persistent_node_survives_reopen() {
        ...
        assert_eq!(
            reopened
                .resolve_node_selector(&gf_core::NodeSelector::Uuid(uuid))
                .unwrap(),
            uuid
        );
    }
    ...
    fn invalid_input_leaves_catalog_and_topology_unchanged() {
        ...
        let catalog_rows = graph
            ...
            .to_record_batch()
            .num_rows();
        let generation = gf_storage::read_topology_generation(&graph.dir).unwrap();
        ...
        assert_eq!(
            gf_storage::read_topology_generation(&graph.dir).unwrap(),
            generation
        );
        assert_eq!(
            gf_storage::read_nodes(&graph.dir)
                .unwrap()
                .iter()
    ...
    fn strict_ontology_accepts_declared_and_rejects_unknown_labels_atomically() {
        ...
        let ontology_path = dir.path().join(gf_core::manifest::ONTOLOGY_FILE);
        std::fs::write(
            &ontology_path,
            "ontology_id: t\nversion: \"v1\"\nentity_types:\n  - name: Person\n    abstract: false\nproperties:\n  - owner: Person\n    name: name\n    type: utf8\n    nullable: true\n  - owner: Person\n    name: score\n    type: int64\n    nullable: true\n",
        )
        ...
        assert_eq!(
            graph
                .resolve_node_selector(&gf_core::NodeSelector::Uuid(handle.uuid))
                .unwrap(),
            handle.uuid
        );
    }
    ...
    fn strict_add_node_rejects_undeclared_properties_before_publication() {
        ...
        let generation_uuid = *graph
            ...
            .lock()
            .expect("generation UUID lock poisoned");
        let topology_generation = gf_storage::read_topology_generation(&graph.dir).unwrap();
        ...
        let rows = gf_storage::read_node_property_rows(&graph.dir, "Host").unwrap();
    ...
    fn undeclared_add_node_properties_remain_permitted_outside_strict_mode() {
        ...
        advisory
            .adopt_ontology(crate::AdoptOntologyRequest {
                ...
                path: ontology_path,
                mode: gf_core::OntologyMode::Advisory,
            })
            .unwrap();
        advisory
            .add_node(
