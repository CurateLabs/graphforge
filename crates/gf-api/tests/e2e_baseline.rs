...
use gf_api::{ExecutionResult, GraphForge};
use gf_ir::IrLiteral;
...
fn parameterized_single_writes_use_the_unified_driver() {
    ...
    let gf = GraphForge::new(None).expect("in-memory instance");
    let counter = |result: &gf_api::ExecutionResult, name: &str| {
...
fn execute_stream_rejects_writes() {
    ...
    match gf.execute_stream("CREATE (:Person {name:'X'})") {
        Err(gf_api::GfError::Validation(_)) => {}
...
fn label_mutations_are_idempotent_counted_and_persistent() {
    let dir = tempfile::TempDir::new().unwrap();
    let path = dir.path().to_str().unwrap();
    let gf = gf_api::GraphForge::new(Some(path)).unwrap();
    ...
    let reopened = gf_api::GraphForge::new(Some(path)).unwrap();
    assert_eq!(
        reopened
...
fn deleted_entities_cannot_be_projected_after_delete() {
    ...
    assert!(
        matches!(error, gf_api::GfError::Execution(message) if message.contains("DeletedEntityAccess"))
    );
...
fn dynamic_subscript_parameter_type_errors_remain_runtime_errors() {
    ...
    assert!(matches!(error, gf_api::GfError::Execution(_)));
}
