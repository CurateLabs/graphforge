...
use gf_core::{FindOptions, NodeSelector};
use gf_exec::ExecutionResult;
use gf_storage::{
...
impl PairedAlgorithmCase {
    ...
    fn execute(
        ...
        source: &NodeSelector,
        target: &NodeSelector,
    ) -> Result<RecordBatch, gf_core::GfError> {
        match self {
...
impl IsolationFixture {
    fn new() -> Self {
        ...
        let bare_data_dir = gf_storage::resolve_project_generation(&bare_dir)
            .expect("resolve bare")
            .participants_root();
        let enriched_data_dir = gf_storage::resolve_project_generation(&enriched_dir)
            .expect("resolve enriched")
            .participants_root();
        let generation = gf_storage::generation::read_topology_generation(&bare_data_dir)
        ...
        assert_eq!(
            gf_storage::generation::read_topology_generation(&bare_data_dir)
                .expect("bare generation after divergence"),
            generation
        );
        assert_eq!(
            gf_storage::generation::read_topology_generation(&enriched_data_dir)
...
fn find_is_identical_across_absent_populated_corrupt_and_future_knowledge() {
    ...
    let corrupt_generation = gf_storage::resolve_project_generation(&corrupt)
    ...
    graph
        .create_assertion(CreateAssertionRequest {
            ...
            graph_refs: vec![AssertionGraphRefInput {
                ...
                graph_kind: gf_api::GraphObjectKind::Node,
                role: gf_api::AssertionGraphRole::Subject,
                ordinal: 0,
            }],
        })
...
fn publish_future_knowledge_generation(root: &Path) {
    let parent = gf_storage::resolve_project_generation(root).expect("resolve future parent");
    ...
    let ProjectStageOutcome::Staged(staged) =
        gf_storage::stage_project_generation(root, &request).expect("stage future knowledge")
    else {
...
fn assert_algorithm_parity(
    algorithm: Algorithm,
    left: Result<RecordBatch, gf_core::GfError>,
    right: Result<RecordBatch, gf_core::GfError>,
...
fn assert_query_parity(
    query: &str,
    left: Result<ExecutionResult, gf_core::GfError>,
    right: Result<ExecutionResult, gf_core::GfError>,
) {
    match (left, right) {
        (Ok(left), Ok(right)) => {
            assert_eq!(
