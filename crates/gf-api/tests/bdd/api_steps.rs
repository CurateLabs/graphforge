...
//! as real implementations land in gf-core milestone by milestone.
...
async fn given_person_node(world: &mut GraphForgeWorld, _name: String) {
    world.forge = Some(gf_api::GraphForge::new(None).expect("in-memory forge must succeed"));
}
...
async fn given_3_persons_knows(world: &mut GraphForgeWorld) {
    let forge = gf_api::GraphForge::new(None).expect("in-memory forge must succeed");
    forge
        .execute(
...
async fn given_named_person_pair(world: &mut GraphForgeWorld, first: String, second: String) {
    ...
    for name in [first, second] {
        let props = std::collections::HashMap::from([(
            "name".to_owned(),
            gf_api::PropValue::Str(name.clone()),
        )]);
        let handle = forge
...
async fn when_add_node_aged(world: &mut GraphForgeWorld, label: String, name: String, age: u32) {
    let props = std::collections::HashMap::from([
        ("name".to_owned(), gf_api::PropValue::Str(name.clone())),
        ("age".to_owned(), gf_api::PropValue::Int(i64::from(age))),
    ]);
    match world
        .forge
        .as_ref()
...
async fn when_paths_with_selector_form(
    ...
    let (source, target) = match selector.as_str() {
        "UUID" => (
            gf_api::NodeSelector::Uuid(alice.uuid),
            gf_api::NodeSelector::Uuid(bob.uuid),
        ),
        "handle" => (
            gf_api::NodeSelector::Handle(alice.clone()),
            gf_api::NodeSelector::Handle(bob.clone()),
        ),
        "property" => (
            gf_api::NodeSelector::Match {
                label: "Person".into(),
                property: "name".into(),
                value: gf_api::PropValue::Str("Alice".into()),
            },
            gf_api::NodeSelector::Match {
                label: "Person".into(),
                property: "name".into(),
                value: gf_api::PropValue::Str("Bob".into()),
            },
        ),
...
async fn when_paths_with_invalid_selector(
    ...
    let source = match case.as_str() {
        "malformed" => match gf_api::NodeSelector::uuid("not-a-uuid") {
            Ok(_) => unreachable!(),
            Err(error) => {
        ...
        "missing" => gf_api::NodeSelector::uuid("01900000-0000-7000-8000-000000000000")
            .expect("valid missing UUID"),
        "ambiguous" => {
            let props = std::collections::HashMap::from([(
                "name".to_owned(),
                gf_api::PropValue::Str("Alice".into()),
            )]);
        ...
        "cross-graph" => {
            let other = gf_api::GraphForge::new(None).expect("foreign graph");
            gf_api::NodeSelector::Handle(
                other
    ...
    world.last_error = forge
        .paths(
            &source,
            Some(&gf_api::NodeSelector::Handle(bob.clone())),
            options,
        )
        .err()
        .map(|error| error.to_string());
...
fn run_rank(
    ...
    let by = match algorithm.parse::<gf_api::RankAlgorithm>() {
        Ok(by) => by,
        Err(error) => {
            world.last_error = Some(error.to_string());
...
async fn when_explain(world: &mut GraphForgeWorld, query: String) {
    match gf_cypher::explain(&query) {
        Ok(s) => {
            world.last_result = Some(gf_api::RecordBatch {
...
async fn when_open_bad_path(world: &mut GraphForgeWorld) {
    match gf_api::GraphForge::new(Some("/nonexistent/path/xyz")) {
