...
pub struct GraphForgeWorld {
    ...
    pub forge: Option<gf_api::GraphForge>,
    ...
    pub last_result: Option<gf_api::RecordBatch>,
    /// Last Arrow-backed result returned by `execute()`.
    pub last_exec: Option<gf_api::ExecutionResult>,
    ...
    pub params: std::collections::HashMap<String, gf_api::IrLiteral>,
    /// Node handles by name.
    pub nodes: std::collections::HashMap<String, gf_api::NodeHandle>,
    /// Most recently created node handle for result-focused assertions.
    pub last_node_handle: Option<gf_api::NodeHandle>,
}
    ...
    if !xpasses.is_empty() {
        eprintln!(
            "::warning title=TCK XPASS::{} scenario(s) now pass that weren't in the baseline — \
             lock them in with `BLESS_TCK_BASELINE=1 cargo test -p gf-api --test bdd`",
            xpasses.len(),
        );
