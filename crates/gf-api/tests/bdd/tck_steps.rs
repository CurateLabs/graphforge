...
async fn given_procedure(world: &mut GraphForgeWorld, signature: String, step: &Step) {
    ...
    world
        ...
        .register_procedure(gf_api::ProcedureDefinition {
            name,
            inputs,
            outputs,
            rows,
...
fn parse_procedure_fields(raw: &str) -> Vec<gf_api::ProcedureField> {
    let raw = raw.trim();
    if raw.is_empty() {
        return vec![];
    }
...
/// as a normal result.
fn is_write_summary(exec: &gf_api::ExecutionResult) -> bool {
...
fn parse_tck_param_literal(raw: &str) -> gf_api::IrLiteral {
...
fn val_to_ir_literal(value: Val) -> gf_api::IrLiteral {
    match value {
        Val::Null => gf_api::IrLiteral::Null,
        Val::Bool(v) => gf_api::IrLiteral::Bool(v),
        Val::Int(v) => gf_api::IrLiteral::Int(v),
        Val::Float(v) => gf_api::IrLiteral::Float(v),
        Val::Str(v) => gf_api::IrLiteral::Str(v),
        Val::List(items) => {
            gf_api::IrLiteral::List(items.into_iter().map(val_to_ir_literal).collect())
        }
        Val::Map(entries) => gf_api::IrLiteral::Map(
            entries
                .into_iter()
    ...
    match array.data_type() {
                ...
                && *fields[0].data_type() == DataType::Int64 =>
        {
            ...
            let days = s
                ...
                .value(row);
            format!("'{}'", gf_rel::temporal::format_date(days))
        }
        ...
        DataType::Time64(TimeUnit::Nanosecond) => {
            ...
            format!(
                "'{}'",
                gf_rel::temporal::render_localtime_nanos(a.value(row))
            )
        }
                ...
                && fields[3].name() == "nanos" =>
        {
            ...
            format!(
                "'{}'",
                gf_rel::temporal::render_duration_value(&gf_rel::temporal::DurationValue {
                    months: col(0),
                    days: col(1),
                    seconds: col(2),
                    nanos: col(3),
                ...
                && *fields[1].data_type() == DataType::Time64(TimeUnit::Nanosecond) =>
        {
            ...
            let nanos = s
                ...
                .expect("time field")
                .value(row);
            format!("'{}'", gf_rel::temporal::render_localdatetime(days, nanos))
        }
                ...
                && *fields[1].data_type() == DataType::Int32 =>
        {
            ...
            let offset = s
                ...
                .expect("offset field")
                .value(row);
            format!("'{}'", gf_rel::temporal::render_time_value(nanos, offset))
        }
                ...
                && fields[3].name() == "zone" =>
        {
            ...
            format!(
                "'{}'",
                gf_rel::temporal::render_datetime_value(days, nanos, offset, zone)
            )
        }
        ...
        DataType::Struct(fields) if fields.iter().any(|f| f.name() == "__het_tag") => {
            ...
            let decoded =
                gf_rel::expr::decode_het_scalar(&value).expect("valid tagged property scalar");
