...
fn configured_session_composes_embedding_query_rerank_and_advisory() {
    ...
    assert!(matches!(
        ...
        Some(gf_api::EmbeddingRefreshOutcomeStatus::Succeeded)
    ));
    let active = graph
        .embedding_space(Some("semantic"))
        .unwrap()
    ...
    graph
        .set_embedding_refresh_project_policy(gf_api::EmbeddingRefreshProjectPolicy {
            proactive: false,
    ...
    assert!(matches!(
        ...
        Some(gf_api::EmbeddingSpaceFreshnessState::SubstantiallyStale)
    ));
}
...
fn proactive_provider_failure_preserves_the_active_generation() {
    ...
    assert!(matches!(
        ...
        Some(gf_api::EmbeddingRefreshOutcomeStatus::Failed(
            gf_api::EmbeddingRefreshFailureClass::Provider
        ))
    ));
    let active = graph
        .embedding_space(Some("semantic"))
