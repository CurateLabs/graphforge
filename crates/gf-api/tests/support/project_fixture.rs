...
use gf_core::canonical::{CANONICAL_CONTRACT_VERSION, CanonicalDomain, fingerprint};
...
pub(crate) fn publish_graph_workspace(container: &Path, workspace: &Path) {
    let parent = gf_storage::open_or_initialize_project(container).unwrap();
    ...
    let request = ProjectGenerationRequest {
        ...
        participants: {
            ...
            participants.extend(gf_storage::empty_workspace_participants().unwrap());
            participants
        },
    };
    let ProjectStageOutcome::Staged(staged) =
        gf_storage::stage_project_generation(container, &request).unwrap()
    else {
