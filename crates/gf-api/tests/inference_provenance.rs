...
fn events(dir: &std::path::Path) -> Vec<(String, Option<String>, Option<String>)> {
    let path = gf_storage::resolve_project_generation(dir)
...
fn seed(dir: &std::path::Path) -> GraphForge {
    gf_storage::open_or_initialize_project(dir).expect("initialize project");
...
fn transitive_match_does_not_publish_mutation_provenance() {
    ...
    let before = gf_storage::resolve_project_generation(dir.path())
        .unwrap()
        .generation_uuid();
    ...
    let after = gf_storage::resolve_project_generation(dir.path())
        .unwrap()
        .generation_uuid();
