...
fn refresh_embeddings(
    ...
    request: &ProviderEmbeddingPlanRequest,
    provider: &mut FakeProvider,
) -> gf_api::EmbeddingRefreshInspection {
    let mut runtime = StandardProviderExecutionRuntime::new();
    let mut count_tokens =
        |_: &ProviderModelContract, text: &str| Ok(u64::try_from(text.len()).unwrap());
    let mut estimate_cost = |shape: ProviderBatchShape| Ok(shape.input_tokens());
...
fn rerank(
    ...
    provider: &mut FakeProvider,
) -> gf_api::ProviderRerankedFindResult {
    let mut runtime = StandardProviderExecutionRuntime::new();
    let mut count_tokens =
        |_: &ProviderModelContract, text: &str| Ok(u64::try_from(text.len()).unwrap());
    let mut estimate_cost = |shape: gf_api::RerankWorkShape| Ok(shape.input_tokens());
    let mut checkpoint = || Ok(());
    graph
        .rerank_find_results(
            canonical,
...
fn assert_reranked(
    reranked: &gf_api::ProviderRerankedFindResult,
    repeated: &gf_api::ProviderRerankedFindResult,
    original: &[u8],
) {
...
fn provider_refresh_and_rerank_public_facades_are_covered() {
    ...
    graph
        .index_search(
            "Document",
            gf_api::SearchIndexOptions::Text {
    ...
    let mut rerank_count_tokens =
        |_: &ProviderModelContract, text: &str| Ok(u64::try_from(text.len()).unwrap());
    let mut rerank_estimate_cost = |shape: gf_api::RerankWorkShape| Ok(shape.input_tokens());
