...
use gf_api::{AdoptOntologyRequest, GfError, GraphForge, OntologyMode, OperationId, WriteContext};
use uuid::Uuid;
...
fn only_batch(result: gf_api::ExecutionResult) -> RecordBatch {
...
fn strict_runtime_properties_are_owner_validated_atomic_and_reopen_stable() {
    ...
    let generation_before_failure = gf_storage::resolve_project_generation(project.path()).unwrap();
    ...
    let generation_after_failure = gf_storage::resolve_project_generation(project.path()).unwrap();
    assert_eq!(
