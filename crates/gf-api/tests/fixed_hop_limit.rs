...
use gf_exec::demand::{self, DemandSnapshot};
use gf_ir::IrLiteral;
use gf_storage::adjacency::build_adjacency_index;
use gf_storage::{GraphWriter, io_stats};
...
fn uint64_values(result: &gf_api::ExecutionResult, column: &str) -> Vec<u64> {
...
fn fixed_binary_values(result: &gf_api::ExecutionResult, column: &str) -> Vec<Vec<u8>> {
    let mut values = Vec::new();
    for batch in &result.batches {
        let array = batch
            .column_by_name(column)
