...
fn record_hypothesis(graph: &GraphForge, subject: Uuid) -> graphforge_api::ExecutionResult {
    let assertion = graph
...
fn strict_graph(project_path: &str) -> GraphForge {
    ...
    assert_eq!(graph.ontology_mode(), graphforge_api::OntologyMode::Strict);
...
fn reopen_equal(graph: GraphForge, project_path: &str) -> bool {
    ...
    assert_eq!(
        ...
        graphforge_api::OntologyMode::Strict
    );
    reopened
        .inspect_text_index("Person", Some(&["name".into(), "summary".into()]))
        .unwrap()
