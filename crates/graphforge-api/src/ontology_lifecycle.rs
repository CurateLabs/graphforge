...
mod tests {
    ...
    fn yaml_and_json_exports_are_atomic_valid_documents() {
        ...
        for (format, name) in [
            ...
            graph
                ...
                .unwrap();
            assert_eq!(
                graphforge_ontology::OntologyLoader::load_file(&path).unwrap(),
                suggestion.document
            );
        }
    }
    ...
    fn export_round_trip_preserves_migration_route_tie_breaking() {
        ...
        let authored = OntologyDoc {
            ...
            constraints: vec![],
            migrations: vec![
                graphforge_ontology::MigrationDef {
                    from_version: "1".into(),
                    to_version: "3".into(),
        ...
        let before = graphforge_ontology::MigrationEngine::plan("1", "4", &authored.migrations)
            .unwrap()
            .into_iter()
        ...
        graph
            ...
            .unwrap();
        let round_tripped = graphforge_ontology::OntologyLoader::load_file(&output).unwrap();
        let after = graphforge_ontology::MigrationEngine::plan("1", "4", &round_tripped.migrations)
            .unwrap()
            .into_iter()
            .map(|step| step.to_version)
            .collect::<Vec<_>>();
    ...
    fn loaded_and_adopted_sources_are_explicit_and_leave_authority_unchanged() {
        ...
        let loaded_doc = graphforge_ontology::OntologyLoader::load_file(&loaded).unwrap();
        let adopted_doc = graphforge_ontology::OntologyLoader::load_file(&adopted).unwrap();
        ...
        reopened
            ...
            .unwrap();
        assert_eq!(
            graphforge_ontology::OntologyLoader::load_file(&reopened_path).unwrap(),
            adopted_doc
        );
    }
