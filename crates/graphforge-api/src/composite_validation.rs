...
use graphforge_core::{ApiErrorCode, GfError, OntologyMode};
...
mod tests {
    ...
    use graphforge_core::PropValue;
    use graphforge_knowledge::{AssertionGraphRef, AssertionGraphRole};
    ...
    fn reference_request() -> CompositeTransactionRequest {
        ...
        for row in &mut subject.knowledge.provenance_events {
            *row = graphforge_provenance::ProvenanceEvent::new(operation, row.event_kind, None, row.recorded_at_micros).unwrap();
        }
        ...
        subject.knowledge.evidence[0] = graphforge_knowledge::EvidenceLink::new(
            evidence.evidence_uuid, assertion, evidence.source_uuid, graphforge_knowledge::EvidenceSourceKind::Observation,
    ...
    fn snapshot_and_request_references_accept_every_lineage_subject_kind() {
        ...
        for (kind, subject_uuid) in cases {
            ...
            subject.knowledge.confidence_inputs.push(graphforge_knowledge::ConfidenceInput::new(
    ...
    fn every_owned_reference_rejects_a_wrong_identity_family() {
        ...
        context.knowledge.provenance_events[0] = graphforge_provenance::ProvenanceEvent::new(wrong, row.event_kind, row.actor_uuid, row.recorded_at_micros).unwrap();
        ...
        actor.knowledge.provenance_events[0] = graphforge_provenance::ProvenanceEvent::new(row.operation_uuid, row.event_kind, Some(wrong), row.recorded_at_micros).unwrap();
        ...
        precedence.knowledge.lineage[0] = graphforge_provenance::LineageRecord::new(
            row.provenance_uuid, row.subject_uuid, row.subject_kind, row.role, row.ordinal,
        ).unwrap();
