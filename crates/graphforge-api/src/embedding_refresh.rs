...
impl GraphForge {
    ...
    fn refresh_worker_handle(&self) -> Self {
        Self {
            ...
            graph_files_publication: Arc::clone(&self.graph_files_publication),
...
fn same_projection(
    recorded: graphforge_storage::EmbeddingSourceState,
    current: graphforge_storage::EmbeddingSourceState,
) -> bool {
...
mod tests {
    ...
    fn defaults_and_space_overrides_are_durable_and_content_free() {
        ...
        let outcome = EmbeddingRefreshOutcomeRecord {
            status: graphforge_storage::EmbeddingRefreshOutcomeStatus::Failed(
                graphforge_storage::EmbeddingRefreshFailureClass::Provider,
            ),
            graph_generation: 1,
            source_fingerprint: graphforge_storage::EmbeddingSourceFingerprint::digest(b"source"),
    ...
    fn project_policy_rebuilds_only_an_idle_bounded_worker() {
        ...
        let source = graphforge_storage::EmbeddingSourceState::new(1, [1; 32], [2; 32], 0);
        graph
            .embedding_refresh_scheduler
            .lock()
            .unwrap()
    ...
    fn policy_validation_aliases_and_cancellation_remain_structured() {
        ...
        let error = graph
            ...
            .enqueue(
                ...
                graphforge_storage::EmbeddingSourceState::new(1, [1; 32], [2; 32], 0),
                Duration::ZERO,
                || Ok(()),
            )
            .unwrap_err();
