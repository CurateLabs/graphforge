...
use graphforge_provenance::{EventKind, LineageRecord, LineageRole, ProvenanceEvent, SubjectKind};
...
fn pre_current_retention_removals(source: &Path) -> BTreeSet<String> {
    ...
    let recovery = graphforge_storage::recover_project_transactions(oracle_root.path())
...
fn post_current_retention_removals(source: &Path) -> BTreeSet<String> {
    ...
    drop(graph);
    let selected = graphforge_storage::resolve_project_generation(oracle_root.path())
...
fn verify_case(failpoint: &str, committed: bool) {
    ...
    let before_generation = graphforge_storage::resolve_project_generation(root.path())
    ...
    let recovery = graphforge_storage::recover_project_transactions(root.path())
    ...
    let resolved = graphforge_storage::resolve_project_generation(root.path())
    ...
    let second_recovery = graphforge_storage::recover_project_transactions(root.path())
        .unwrap_or_else(|error| {
