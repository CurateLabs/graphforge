...
use graphforge_storage::embedding_catalog::remove_embedding_space_catalog_entry;
...
fn join_spaces(
    catalog: &graphforge_storage::EmbeddingSpaceCatalog,
    discovered: &[graphforge_storage::DiscoveredEmbeddingSpace],
) -> Result<Vec<EmbeddingSpaceInfo>, GfError> {
    let discovered_ids = discovered
        .iter()
        .map(graphforge_storage::DiscoveredEmbeddingSpace::compatibility_id)
        .collect::<BTreeSet<_>>();
