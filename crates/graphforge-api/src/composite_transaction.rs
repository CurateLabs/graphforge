...
use graphforge_provenance::{
    LineageRecord, MAX_PROVENANCE_ROWS, ProvenanceEvent, ProvenanceLedger,
};
...
fn canonical_error(error: graphforge_core::canonical::CanonicalError) -> GfError {
...
const fn provenance_role_order(role: graphforge_provenance::LineageRole) -> u8 {
    match role {
        graphforge_provenance::LineageRole::Input => 0,
        graphforge_provenance::LineageRole::Output => 1,
    }
}
...
pub(crate) mod tests {
    ...
    use graphforge_provenance::{EventKind, LineageRole, SubjectKind};
    ...
    fn inventory_and_independent_domain_limits_are_frozen() {
        ...
        assert_eq!(graphforge_knowledge::MAX_KNOWLEDGE_ROWS, 1_000_000);
        assert_eq!(graphforge_provenance::MAX_PROVENANCE_ROWS, 1_000_000);
        assert_eq!(graphforge_knowledge::MAX_REASONING_CONTENT_BYTES, 65_536);
        assert_eq!(graphforge_knowledge::MAX_HYPOTHESIS_QUESTION_KEY_BYTES, 1_024);
        assert_eq!(
