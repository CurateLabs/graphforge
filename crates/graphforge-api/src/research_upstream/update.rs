...
impl GraphForge {
    ...
    pub fn update_research_branch(
        ...
        let source_capture = source_capture(&preview, request)?;
        ...
        explain(&mut graph, &selected, request, cancel)?;
        ...
        let fields = reviewed_fields(&preview, &selected, &current);
...
fn explain(
    ...
    selected: &std::collections::BTreeMap<fields::Key, ResearchUpstreamResolution>,
    request: &UpdateResearchBranchRequest,
    cancel: &CancellationToken,
) -> Result<(), GfError> {
...
) -> Vec<ResearchUpstreamFieldReview> {
