...
impl GraphForge {
    ...
    pub fn attach_resolved_run(
        ...
        if terminal.state != graphforge_knowledge::AlgorithmRunState::Completed {
...
fn one_batch<'a>(
    result: &'a graphforge_exec::ExecutionResult,
    name: &str,
...
fn fail_attachment_for_test() -> Result<(), GfError> {
    INJECT_ATTACHMENT_FAILURE.with(|enabled| {
        if enabled.get() {
            Err(GfError::Project {
                code: graphforge_core::ProjectErrorCode::PublicationFailed,
...
mod tests {
    ...
    use graphforge_core::algorithms::ClusterAlgorithm;
    ...
    fn statusless_include_materializes_the_same_neutral_rank_projection() {
        ...
        let current_generation = graphforge_storage::resolve_project_generation(
            graph.resolved_generation.container_root(),
        )
        .unwrap();
        ...
        assert_eq!(
            graphforge_storage::read_nodes(empty_target.path())
                .unwrap()
                .iter()
                .map(RecordBatch::num_rows)
                .sum::<usize>(),
