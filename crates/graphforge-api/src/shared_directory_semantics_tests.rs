...
fn publication_request(root: &std::path::Path) -> ProjectGenerationRequest {
    let selected =
        graphforge_storage::resolve_project_generation(root).expect("phase=writer-lock resolve");
...
fn shared_directory_reads_pin_complete_generations_and_reopen_sees_commit() {
    ...
    let committed = graphforge_storage::resolve_project_generation(project.path())
...
fn competing_writer_fails_before_its_staging_or_publication() {
    ...
    let holder = match graphforge_storage::stage_project_generation(project.path(), &holder_request)
    ...
    let worker = {
        ...
        thread::spawn(move || {
            barrier.wait();
            let result = graphforge_storage::stage_project_generation(&root, &contender_request)
                .map(|_| ())
    ...
    let observed = graphforge_storage::resolve_project_generation(project.path())
