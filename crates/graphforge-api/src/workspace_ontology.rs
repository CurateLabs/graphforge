...
pub(crate) fn publish_workspace_records(
    ...
    let mut participants = parent
        ...
        .into_iter()
        .filter(|snapshot| {
            !(snapshot.capability_id == graphforge_storage::WORKSPACE_CAPABILITY_ID
                && matches!(
...
fn validate_workspace_record_inventory(
    metadata: &[graphforge_storage::StagedParticipant],
) -> Result<(), GfError> {
...
mod tests {
    ...
    use graphforge_core::{RankOptions, algorithms::RankAlgorithm};
    ...
    fn staged_workspace(family: &str) -> graphforge_storage::StagedParticipant {
