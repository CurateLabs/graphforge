...
fn main() {
    ...
    generated.push_str("const PROJECT_SKILL_FILES: &[gf_api::SkillBundleFile<'static>] = &[\n");
    for path in files {
        let relative = path
            .strip_prefix(&root)
            .expect("project skill remains under canonical root")
        ...
        writeln!(
            generated,
            "    gf_api::SkillBundleFile {{ path: {relative_literal}, bytes: include_bytes!({path_literal}) }},"
        )
