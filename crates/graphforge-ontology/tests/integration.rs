...
use graphforge_ontology::error::OntologyError;
use graphforge_ontology::migration::MigrationEngine;
...
fn parquet_round_trip_preserves_property_owner_namespaces() {
    ...
    assert_eq!(
        ...
        Some(graphforge_ontology::PropId(0))
    );
    assert_eq!(
        handle.relation_property_type_id(connected_to, "shared"),
        Some(graphforge_ontology::PropId(1))
    );
    assert_eq!(
