...
use gf_storage::adjacency_delta::{DeltaSegment, apply_delta_segments, read_delta_chain};
use gf_storage::generation::read_topology_generation;
...
/// Scan-build provider: `gf_storage::read_edges` + in-memory build on every
...
impl AdjacencyProvider for ScanBuildAdjacencyProvider {
    fn adjacency(
        ...
        let batches = gf_storage::read_edges(&self.dir, read_name, self.mode)
...
/// `gf_storage::adjacency::read_csr`, which validates the invariants
...
mod tests {
    ...
    fn post_delete_sparse_ids() {
        ...
        let incident = gf_storage::incident_edge_uuids(dir.path(), &node_set).unwrap();
        let edge_set: std::collections::HashSet<[u8; 16]> = incident.into_iter().collect();
        gf_storage::delete_nodes_and_edges(dir.path(), &node_set, &edge_set).unwrap();
