...
pub use gf_core::GfError;
use gf_core::OntologyMode;
pub use gf_ir::GraphPlan;
use gf_ir::{Direction, ExprArena, ExprId, GraphOp, IrExpr, IrLiteral, VarId};
...
use gf_rel::{GraphPlanLowerer, scalar_to_ir_literal};
...
/// Graph-native mutation semantics emitted by `gf-exec`.
///
/// This closed registry deliberately contains no provenance or knowledge
/// vocabulary. `gf-api` is responsible for translating these neutral effects
/// into domain records.
...
///
/// Drives a [`gf_storage::GraphWriter`] **once per input row** (#703): for each
    ...
    fn execute(
        ...
        let fut = async move {
            ...
            let persisted_ids = cfg
                ...
                .map_err(to_df_err)?;
            let mut writer =
                gf_storage::GraphWriter::open(&cfg.dir, cfg.mode).map_err(to_df_err)?;
...
fn validate_edge_specs(cfg: &CreateConfig) -> Result<(), GfError> {
    for spec in &cfg.edges {
        ...
        if matches!(spec.direction, gf_ir::Direction::Undirected) {
            return Err(GfError::Execution(
...
fn persisted_node_ids(dir: &Path) -> Result<std::collections::HashMap<[u8; 16], u64>, GfError> {
    ...
    for batch in gf_storage::read_nodes(dir).map_err(|e| GfError::Storage(e.to_string()))? {
        let uuids = batch
...
fn referenced_node_uuid(
    ...
    cols: &RefNodeCols,
    row: usize,
) -> Result<gf_core::uuid::Uuid, GfError> {
...
fn count_set_props(props: &std::collections::HashMap<String, gf_ir::IrLiteral>) -> u64 {
    props
        .values()
        .filter(|v| !matches!(v, gf_ir::IrLiteral::Null))
        .count() as u64
}
...
fn merge_computed(
    ...
    var: u32,
    row: usize,
    props: &mut std::collections::HashMap<String, gf_ir::IrLiteral>,
) -> Result<(), GfError> {
    ...
    for row in 0..batch.num_rows() {
        ...
        for spec in &cfg.nodes {
            if spec.is_reference {
                ...
                var_to_uuid.insert(spec.var, uuid);
                let mut props: HashMap<String, gf_ir::IrLiteral> =
...
fn resolved_node_labels(spec: &ResolvedNodeSpec) -> (Vec<gf_core::TypeId>, gf_core::TypeId) {
    let labels: Vec<_> = spec
        ...
        .map(gf_core::TypeId)
        .collect();
    let primary = labels.first().copied().unwrap_or(gf_core::TypeId(u32::MAX));
    (labels, primary)
}
...
fn append_created_node_output_cols(
    ...
    for (_, lit) in &spec.properties {
        let scalar = gf_rel::expr::ir_literal_to_scalar(lit);
        out_cols.push(
            scalar
...
fn fixed_binary_uuid(
    ...
    Ok(gf_core::uuid::from_bytes(&bytes))
}
...
/// [`gf_storage::mutator`] primitives. Emits a one-row summary batch
...
impl ExecutionPlan for GraphDeleteExec {
    ...
    fn execute(
        ...
        let fut = async move {
            ...
            let incident = gf_storage::incident_edge_uuids(&dir, &node_uuids).map_err(to_df_err)?;
            if detach {
            ...
            let (nodes_deleted, edges_deleted) =
                gf_storage::delete_nodes_and_edges(&dir, &node_uuids, &edge_uuids)
                    .map_err(to_df_err)?;
...
fn collect_delete_targets(
    ...
    for col in cols {
        ...
        for row in 0..batch.num_rows() {
            ...
            let uuid = gf_core::uuid::to_bytes(&fixed_binary_uuid(batch, col.uuid_idx, row)?);
            if col.is_edge {
...
impl SetAccumulator {
    ...
    fn stage_into(
        &self,
        staged: &mut gf_storage::RewriteBatch,
        dir: &Path,
    ) -> Result<u64, GfError> {
        let mut total = 0u64;
        for (stem, updates) in &self.nodes {
            total += gf_storage::stage_set_node_properties(staged, dir, stem, updates)?;
        }
        for (stem, updates) in &self.edges {
            total += gf_storage::stage_set_edge_properties(staged, dir, stem, updates)?;
        }
        Ok(total)
    }
    ...
    fn apply(&self, dir: &Path) -> Result<u64, GfError> {
        let mut staged = gf_storage::RewriteBatch::new();
...
impl RemoveAccumulator {
    ...
    fn stage_into(
        ...
        for (stem, removals) in &self.nodes {
            total += gf_storage::stage_remove_node_properties(staged, dir, stem, removals)?;
        }
        for (stem, removals) in &self.edges {
            total += gf_storage::stage_remove_edge_properties(staged, dir, stem, removals)?;
        }
        Ok(total)
    }
...
/// affected property files via the [`gf_storage`] SET primitives once. Emits a
...
/// rewrites the affected property files via the [`gf_storage`] REMOVE primitives
...
/// [`gf_storage::read_edges`], since the DataFusion `TaskContext` exposes no
...
/// destination node's [`TOPOLOGY_NODES_SCHEMA`](gf_storage) columns.  The edge
...
/// Pass-through physical node for [`OntologyInferNode`](gf_plan::OntologyInferNode)
...
impl OntologyInferExec {
    ...
    #[must_use]
    pub fn new(node: &gf_plan::OntologyInferNode, input: Arc<dyn ExecutionPlan>) -> Self {
        let schema: SchemaRef = Arc::new(node.schema().as_arrow().clone());
        let props = Arc::new(PlanProperties::new(
            EquivalenceProperties::new(schema.clone()),
            Partitioning::UnknownPartitioning(1),
...
fn expand_bfs(cfg: &ExpandConfig, input_batches: &[RecordBatch]) -> Result<RecordBatch, GfError> {
    ...
    // opens the file).
    let edge_batches =
        gf_storage::read_edges_filtered(&cfg.dir, &cfg.rel_type_name, cfg.mode, &traversed)
    ...
    let node_batches =
        gf_storage::read_nodes_filtered(&cfg.dir, &reached).map_err(|e| exec_err(e.to_string()))?;
...
///
/// Edge column layout (`gf-storage` schemas): edge_uuid=0, src_uuid=1,
...
fn build_edge_prop_children(
    ...
    let stems: Vec<String> = if rel_type_name == "*" {
        gf_storage::list_edge_property_stems(dir)
    } else {
    ...
    for stem in &stems {
        let batches =
            gf_storage::read_edge_properties(dir, stem).map_err(|e| exec_err(e.to_string()))?;
...
/// counterpart of [`gf_plan::ExpandNode`].
///
...
/// the [`gf_plan::ExpandNode`] schema order — the same rows the join chain
/// produces.
...
fn expand_single_hop_chunk(
    ...
    let edge_observer = demand::capture_enabled().then(|| {
        Arc::new(demand::HopReadObserver::new(cfg.edge_var))
            as Arc<dyn gf_storage::io_stats::FilteredReadObserver>
    });
    let edge_batches = gf_storage::read_edges_filtered_observed(
        &cfg.dir,
        &cfg.rel_type_name,
        cfg.mode,
        &traversed,
    ...
    let node_batches =
        gf_storage::read_nodes_filtered_observed(&cfg.dir, &reached, node_observer.as_ref())
            .map_err(|e| exec_err(e.to_string()))?;
    drop(node_permit);
    let Some(first) = node_batches.first() else {
        return Ok(RecordBatch::new_empty(cfg.out_schema.clone()));
...
impl ExtensionPlanner for GraphForgeExtensionPlanner {
    async fn plan_extension(
        ...
        if let Some(expand) = node.as_any().downcast_ref::<gf_plan::ExpandNode>() {
            let input = physical_inputs.first().cloned().ok_or_else(|| {
        ...
        if let Some(infer) = node.as_any().downcast_ref::<gf_plan::OntologyInferNode>() {
...
impl ExecutionSession {
    ...
    pub async fn execute_create_with_params(
        &self,
        plan: &GraphPlan,
        params: &HashMap<String, gf_ir::IrLiteral>,
    ) -> Result<ExecutionResult, GfError> {
        if !plan
            .ops
            .iter()
            .any(|op| matches!(op, gf_ir::GraphOp::Merge { .. }))
        {
    ...
    pub async fn execute_write_statement_with_params(
        ...
        let mut var_map = gf_rel::VarMap::new();
        let logical =
    ...
    async fn resolve_row_count_expressions(
        ...
        for op in &mut resolved.ops {
            ...
            let vars = gf_rel::VarMap::new();
            let lowerer = gf_rel::ExprLowerer::new(&exprs, self.ontology.as_ref(), &vars);
            let expr = lowerer
                .lower(expr)
...
fn bind_query_params(
    ...
    let values: HashMap<String, datafusion::scalar::ScalarValue> = params
        .iter()
        .map(|(name, lit)| (name.clone(), gf_rel::ir_literal_to_scalar(lit)))
        .collect();
    logical
...
mod tests {
    ...
    async fn row_count_expressions_resolve_inside_union_inputs() {
        ...
        let one = child.push_expr(IrExpr::Literal(gf_ir::IrLiteral::Int(1)));
        let two = child.push_expr(IrExpr::BinaryOp {
            op: gf_ir::BinaryOpKind::Add,
            left: one,
            right: one,
        });
