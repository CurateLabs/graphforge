...
pub(crate) fn load_node_vectors(
    ...
    for stem in property_stems {
        for (uuid, row) in gf_storage::read_node_property_rows(dir, stem).map_err(storage_error)? {
...
pub(crate) fn load_node_feature_properties(
    ...
    for property in properties {
        ...
        for stem in gf_storage::list_property_stems(dir) {
            for (uuid, row) in
                gf_storage::read_node_property_rows(dir, &stem).map_err(storage_error)?
            {
                if !graph.node_id_by_uuid.contains_key(&uuid) {
                    continue;
                }
...
pub(crate) fn load_node_partition_property(
    ...
    for stem in gf_storage::list_property_stems(dir) {
        rows.extend(gf_storage::read_node_property_rows(dir, &stem).map_err(storage_error)?);
    }
...
fn selected_nodes(dir: &Path, label: Option<TypeId>) -> Result<(Vec<u64>, NodeUuidMap), GfError> {
    let mut rows = Vec::new();
    for batch in gf_storage::read_nodes(dir).map_err(storage_error)? {
...
fn selected_edge_uuids(
    ...
    for batch in
        gf_storage::read_edges_filtered(dir, read_name, mode, edge_ids).map_err(storage_error)?
    {
...
fn selected_weights(
    ...
    let mut out = HashMap::new();
    for stem in stems {
        for batch in gf_storage::read_edge_properties(dir, &stem).map_err(storage_error)? {
            let uuids = fixed_binary(&batch, "edge_uuid")?;
...
fn uuid_text(value: &[u8; 16]) -> String {
    gf_core::uuid::to_string(&gf_core::uuid::from_bytes(value))
}
...
mod tests {
    ...
    fn missing_and_ragged_vectors_fail_without_partial_attachment() {
        ...
        gf_storage::set_node_properties(
            fixture.dir.path(),
            "Person",
            &HashMap::from([(
    ...
    fn stale_persistent_index_falls_back_without_semantic_drift() {
        let fixture = fixture();
        gf_storage::adjacency::build_adjacency_index(fixture.dir.path(), TS).unwrap();
