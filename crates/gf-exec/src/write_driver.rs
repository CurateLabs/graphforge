...
//! disk in **one** [`RewriteBatch`](gf_storage::RewriteBatch) commit at
...
use gf_core::uuid::to_bytes;
use gf_core::{GfError, OntologyMode, TypeId};
use gf_ir::plan::GraphOp;
use gf_ir::{
    CreatePattern, Direction, ExprArena, ExprId, IrExpr, MergeSetItem, SetMapItem, SetPropItem,
    VarId,
};
use gf_plan::{ResolvedEdgeSpec, ResolvedNodeSpec};
use gf_rel::expr::scalar_to_ir_literal;
use gf_rel::{GraphPlanLowerer, VarMap};
...
///
/// One [`GraphWriter`](gf_storage::GraphWriter) buffers every CREATE in the
...
pub(crate) struct StatementWriteContext {
    pub writer: gf_storage::GraphWriter,
...
impl StatementWriteContext {
    ...
    pub(crate) fn new(dir: &Path, mode: OntologyMode) -> Result<Self, GfError> {
        ...
        for batch in
            gf_storage::read_nodes(dir).map_err(|error| GfError::Storage(error.to_string()))?
        {
        ...
        Ok(Self {
            writer: gf_storage::GraphWriter::open(dir, mode)?,
...
impl Frontier {
    ...
    pub(crate) fn append_created_node_var(
        ...
        for (name, lit) in &spec.properties {
            fields.push(Field::new(
                name,
                gf_rel::expr::ir_literal_to_scalar(lit).data_type(),
                true,
            ));
        }
    ...
    fn append_merged_node_rows(
        ...
        for row in rows {
            for (name, value) in &row.properties {
                let data_type = gf_rel::expr::ir_literal_to_scalar(value).data_type();
            ...
            for (name, data_type) in &properties {
                let values = selected.iter().map(|row| {
                    row.properties.get(name).map_or_else(
                        ...
                        |value| Ok(gf_rel::expr::ir_literal_to_scalar(value)),
                    )
                });
                let values = values
                    .collect::<Result<Vec<_>, _>>()
    ...
    fn append_created_edge_var(
        ...
        self.append_var_columns(spec.var, fields, |batch_idx, range| {
            ...
            for (_, lit) in &spec.properties {
                columns.push(
                    gf_rel::expr::ir_literal_to_scalar(lit)
                        .to_array_of_size(rows)
                        .map_err(|e| GfError::Execution(e.to_string()))?,
                );
            }
    ...
    fn append_merged_edge_rows(
        ...
        for row in rows {
            for (name, value) in &row.properties {
                property_types
                    .entry(name.clone())
                    .or_insert_with(|| gf_rel::expr::ir_literal_to_scalar(value).data_type());
            }
        }
        let mut properties = property_types.into_iter().collect::<Vec<_>>();
        properties.sort_unstable_by(|(left, _), (right, _)| left.cmp(right));
...
pub(crate) struct PhaseEnv<'a> {
    ...
    pub params: &'a HashMap<String, gf_ir::IrLiteral>,
        ...
        Ok(params.get(name).map_or_else(
            ...
            |value| {
                Transformed::yes(DfExpr::Literal(
                    gf_rel::expr::ir_literal_to_scalar(value),
                    None,
                ))
            },
        ))
...
fn run_create_phase(
    ...
    let cfg = CreateConfig {
        ...
        mode: env.mode,
        out_schema: gf_plan::GraphCreateNode::summary_schema(),
    };
...
struct MatchedMergeNode {
    ...
    type_id: u32,
    label_ids: Vec<u32>,
    properties: HashMap<String, gf_ir::IrLiteral>,
}
...
fn find_matching_merge_nodes(
    ...
    let mut matches = writer
        ...
        .collect::<Vec<_>>();
    let batches = gf_storage::read_nodes(env.dir).map_err(|e| GfError::Storage(e.to_string()))?;
    for batch in batches {
        ...
        for row in 0..batch.num_rows() {
            ...
            let props = gf_storage::read_entity_properties(env.dir, stem, &uuid, false)?;
            if spec
                .properties
                .iter()
...
fn run_relationship_merge_phase(
    ...
    let edge_batches = gf_storage::read_edges(env.dir, rel_name, env.mode)
    ...
    for batch in &frontier.batches {
        for row in 0..batch.num_rows() {
            ...
            let edge_value = gf_core::uuid::new_v7();
            ctx.writer
            ...
            edge_rows.push(MatchedMergeEdge {
                uuid: gf_core::uuid::to_bytes(&edge_value),
                src_uuid: src_bytes,
                dst_uuid: dst_bytes,
...
fn reject_null_merge_properties(properties: &[(String, gf_ir::IrLiteral)]) -> Result<(), GfError> {
    if let Some((name, _)) = properties
        .iter()
        .find(|(_, value)| matches!(value, gf_ir::IrLiteral::Null))
    {
        return Err(GfError::Execution(format!(
...
fn find_matching_merge_edges(
    ...
    for batch in batches {
        ...
        for row in 0..batch.num_rows() {
            ...
            let props = gf_storage::read_entity_properties(env.dir, rel_name, &uuid, true)?;
            if spec
                .properties
                .iter()
                .all(|(name, value)| props.get(name) == Some(value))
...
fn collect_delete_scalar(
    ...
    if let Some(decoded) = gf_rel::expr::decode_het_scalar(value) {
...
fn run_delete_phase(
    ...
    let mut incident: HashSet<[u8; 16]> =
        gf_storage::incident_edge_uuids(env.dir, &committed_nodes)?
            .into_iter()
            .collect();
    ...
    ctx.counters.properties_removed +=
        gf_storage::count_entity_properties(env.dir, &committed_edges, true)?;
    ...
    // Nodes, likewise.
    ctx.counters.properties_removed +=
        gf_storage::count_entity_properties(env.dir, &committed_nodes, false)?;
...
fn persisted_node_labels(
    ...
    for batch in gf_storage::read_nodes(dir).map_err(|error| GfError::Storage(error.to_string()))? {
            ...
            if maps.column_by_name("__het_tag").is_some() {
                for row in 0..batch.num_rows() {
                    ...
                    if item.replace
                        ...
                        && !ctx.writer.contains_pending_edge(&uuid)
                    {
                        present.extend(gf_storage::read_entity_property_keys(
...
fn decode_tagged_map_updates(
    maps: &StructArray,
    row: usize,
) -> Result<HashMap<String, gf_ir::IrLiteral>, GfError> {
    let tagged = ScalarValue::try_from_array(maps, row)
        .map_err(|error| GfError::Execution(error.to_string()))?;
    let decoded = gf_rel::expr::decode_het_scalar(&tagged)
    ...
    for (field, column) in values.fields().iter().zip(values.columns()) {
        let tagged_value = ScalarValue::try_from_array(column, 0)
            .map_err(|error| GfError::Execution(error.to_string()))?;
        let value = gf_rel::expr::decode_het_scalar(&tagged_value).unwrap_or(tagged_value);
        if !value.is_null() {
            updates.insert(
                field.name().clone(),
...
fn map_removals(
    ...
    updates: &HashMap<String, gf_ir::IrLiteral>,
...
pub(crate) fn commit_statement(ctx: &mut StatementWriteContext, dir: &Path) -> Result<(), GfError> {
    ...
    gf_storage::stage_mutate_node_labels(
        &mut staged,
        dir,
        &ctx.label_additions,
        &ctx.label_removals,
    )?;
    gf_storage::stage_delete_edges(&mut staged, dir, &ctx.pending_edge_deletes)?;
    gf_storage::stage_delete_nodes(&mut staged, dir, &ctx.pending_node_deletes)?;
    ...
    if let Some(generation) = gf_storage::commit_topology_aware(staged, dir)? {
        if pure_append {
            ctx.writer.write_segment_best_effort(generation, &pending);
        } else {
            gf_storage::adjacency_delta::discard_segment(dir, generation);
        }
    }
    Ok(())
}
...
mod tests {
    ...
    use gf_ir::{CreateEdgeSpec, CreateNodeSpec, CreatePattern, VarId};
