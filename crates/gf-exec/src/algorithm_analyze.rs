...
use gf_core::embedding_options::{EmbeddingAnalyzeOptions, EmbeddingOptions};
use gf_core::{AnalyzeOptions, GfError, OntologyMode, TypeId};
use gf_ir::{Direction, IrLiteral};
...
const BUILTIN_REVIEW: DependencyReview = DependencyReview {
    implementation: "gf-exec built-in",
    license: "Apache-2.0",
...
fn execute_hashgnn(
    graph: &AdjacencyGraph,
    options: &gf_core::embedding_options::HashGnnOptions,
...
fn load_hashgnn_node_types(
    ...
    let mut kind = None;
    for stem in gf_storage::list_property_stems(dir) {
        for (uuid, row) in gf_storage::read_node_property_rows(dir, &stem)
...
fn load_hashgnn_relationship_types(
    ...
    for stem in gf_storage::list_edge_property_stems(dir) {
        for batch in gf_storage::read_edge_properties(dir, &stem)
            .map_err(|error| GfError::Storage(error.to_string()))?
        {
...
fn execute_graphsage(
    graph: &AdjacencyGraph,
    options: &gf_core::embedding_options::GraphSageOptions,
    topology: TopologyResources,
    control: &EmbeddingControl<'_>,
) -> Result<Vec<crate::algorithm_embedding_output::EmbeddingOutputRow>, GfError> {
    if graph.is_empty() {
...
mod tests {
    ...
    fn node2vec_invocation(
        options: gf_core::embedding_options::Node2VecOptions,
    ...
    fn fastrp_invocation(
        options: gf_core::embedding_options::FastRpOptions,
    ) -> EmbeddingAnalyzeOptions {
        EmbeddingAnalyzeOptions {
    ...
    fn node2vec_descriptor_is_deterministic_and_persistence_independent() {
        ...
        let options = gf_core::embedding_options::Node2VecOptions {
            dimensions: 4,
            walk_length: 3,
            walks_per_node: 2,
            window_size: 1,
            ...
            epochs: 1,
            seed: 7,
            ..gf_core::embedding_options::Node2VecOptions::default()
        };
        let invocation =
    ...
    fn node2vec_dispatch_resource_failures_are_structured_and_atomic() {
        ...
        for (name, result, expected) in [
            ...
            (
                ...
                execute_node2vec_with_controls(
                    &graph,
                    gf_core::embedding_options::Node2VecOptions {
    ...
    fn fastrp_descriptor_replays_and_dispatch_controls_fail_atomically() {
        let graph = AdjacencyGraph::with_test_directed_edges(3, &[(0, 1), (1, 2)]);
        let options = gf_core::embedding_options::FastRpOptions {
            dimensions: 4,
            iteration_weights: vec![1.0, 1.0],
            seed: 11,
            ..gf_core::embedding_options::FastRpOptions::default()
        };
    ...
    fn graphsage_descriptor_replays_and_dispatch_controls_fail_atomically() {
        ...
        graph
            ...
            .unwrap();
        let options = gf_core::embedding_options::GraphSageOptions {
            dimensions: 2,
            hidden_dimensions: 2,
            layers: 1,
            sample_sizes: vec![1],
            ...
            seed: 13,
            ..gf_core::embedding_options::GraphSageOptions::default()
        };
        let invocation =
    ...
    fn hashgnn_descriptor_replays_and_dispatch_controls_fail_atomically() {
        let graph = AdjacencyGraph::with_test_directed_edges(3, &[(0, 1), (1, 2)]);
        let options = gf_core::embedding_options::HashGnnOptions {
            dimensions: 8,
            iterations: 2,
            ...
            seed: 19,
            ..gf_core::embedding_options::HashGnnOptions::default()
        };
