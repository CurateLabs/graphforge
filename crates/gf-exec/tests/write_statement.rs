...
//! against the session (the gf-api router swap is the next slice).
...
use gf_core::GfError;
use gf_exec::{ExecutionResult, ExecutionSession, MutationKind, MutationSubjectKind};
...
async fn remove_absent_property_is_an_explicit_noop_receipt() {
    ...
    assert!(
        result
            .mutation_receipt
            .as_ref()
            .is_some_and(gf_exec::MutationReceipt::is_empty)
    );
}
...
async fn statement_commit_bumps_topology_generation_only_for_topology() {
    ...
    assert_eq!(
        gf_storage::read_topology_generation(dir.path()).unwrap(),
        1,
    ...
    .await
    .unwrap();
    assert_eq!(gf_storage::read_topology_generation(dir.path()).unwrap(), 2);
    ...
    .await
    .unwrap();
    assert_eq!(gf_storage::read_topology_generation(dir.path()).unwrap(), 3);
}
