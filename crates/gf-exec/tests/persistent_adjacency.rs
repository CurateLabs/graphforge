...
fn stale_via_delete(dir: &Path) {
    ...
    gf_storage::delete_edges(dir, &victims).unwrap();
}
...
fn fresh_index_with_unknown_rel_scan_builds_without_rebuild() {
    ...
    let manifest = gf_storage::adjacency::read_manifest(dir.path()).unwrap();
...
fn corrupt_generation_counter_is_miss_without_rebuild() {
    ...
    build_adjacency_index(dir.path(), TS).unwrap();
    std::fs::write(
        gf_storage::generation::generation_path(dir.path()),
        "not json",
    )
    .unwrap();
...
fn missing_csr_file_with_fresh_manifest_rebuilds_and_repairs() {
    ...
    write_diamond(dir.path());
    build_adjacency_index(dir.path(), TS).unwrap();
    std::fs::remove_file(gf_storage::adjacency::csr_path(
        dir.path(),
        "KNOWS",
        gf_storage::adjacency::Direction::Out,
    ))
    .unwrap();
    ...
    assert!(
        gf_storage::adjacency::csr_path(dir.path(), "KNOWS", gf_storage::adjacency::Direction::Out)
            .exists()
    );
}
...
#[test]
#[ignore = "timing benchmark — run manually with: cargo test -p gf-exec --test persistent_adjacency -- --ignored --nocapture"]
fn load_is_faster_than_rebuild() {
    ...
    let time = |f: &dyn Fn() -> Arc<gf_exec::Adjacency>| {
...
mod session_wiring {
    ...
    use gf_ir::{Binder, GraphPlan, RuntimeCatalog};
    ...
    async fn traversal_results_identical_before_and_after_index_build() {
        ...
        let rows = |result: &gf_exec::ExecutionResult| result.stats.rows_produced;
...
async fn zero_hop_traversal_on_hit_never_opens_edge_files() {
    ...
    use gf_exec::ExecutionSession;
    use gf_ir::{Binder, RuntimeCatalog};
    use gf_storage::GraphCatalog;
    ...
    let bind = |query: &str| {
        ...
        binder
            .bind(&gf_cypher::parse(query).expect("parse"))
            .expect("bind")
    };
    let session = || {
...
fn revalidate_drops_cache_on_external_generation_bump() {
    ...
    gf_storage::generation::bump_topology_generation(dir.path()).unwrap();
...
fn delta_overlay_serves_new_edges_without_rebuild() {
    ...
    write_diamond(dir.path());
    build_adjacency_index(dir.path(), TS).unwrap();
    let base_gen = gf_storage::read_topology_generation(dir.path()).unwrap();
    ...
    w.flush().unwrap();
    assert!(gf_storage::read_topology_generation(dir.path()).unwrap() > base_gen);
