...
fn total_neighbors_thread_matrix_matches_one_thread_bits_and_ordering() {
    ...
    .unwrap();
    assert!(estimated_total_neighbors_work(&neighbors) >= TOTAL_NEIGHBORS_PARALLEL_CROSSOVER_WORK);
...
fn measure_total_neighbors_parallel_crossover() {
    ...
    for (nodes, fanout) in [
        ...
        for _ in 0..5 {
            let t0 = Instant::now();
            let serial = total_neighbor_scores_serial(&neighbors, &degrees, &serial_ctl).unwrap();
...
fn total_neighbors_uses_shared_controls_and_dependency_metadata() {
    ...
    let capability = registry
        .capabilities()
        .into_iter()
        .find(|capability| capability.algorithm == Algorithm::Rank(RankAlgorithm::TotalNeighbors))
        .unwrap();
