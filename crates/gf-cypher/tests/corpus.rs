...
//!
//! A direct Rust regression suite that guards `gf_cypher::parse` against
...
//!
//!    ```bash
//!    BLESS_CORPUS=1 cargo test -p gf-cypher --test corpus
//!    ```
//!
...
use gf_cypher::{ParseErrorKind, parse};
...
fn load_valid_corpus() -> Vec<String> {
    let raw = fs::read_to_string(valid_json_path()).unwrap_or_else(|e| {
        panic!(
            "missing {} ({e}); regenerate with BLESS_CORPUS=1 cargo test -p gf-cypher --test corpus",
            valid_json_path().display()
        )
    });
