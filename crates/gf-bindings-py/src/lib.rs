...
fn text_index_inspection_to_python(
    ...
    value.set_item(
        ...
        inspection
            .reason
            .map(gf_api::TextIndexFreshnessReason::as_str),
    )?;
    Ok(value.into_any().unbind())
}
...
fn adjacency_inspection_to_python(
    py: Python<'_>,
    inspection: gf_api::AdjacencyInspection,
) -> PyResult<Py<PyAny>> {
    let value = PyDict::new(py);
    value.set_item(
    ...
    value.set_item(
        ...
        inspection
            .reason
            .map(gf_api::AdjacencyFreshnessReason::as_str),
    )?;
    value.set_item(
...
fn assertion_status(value: &str) -> Result<gf_api::AssertionStatus, GfError> {
    match value {
...
fn py_assertion_graph_ref(
    py: Python<'_>,
    value: &Bound<'_, PyAny>,
) -> PyResult<gf_api::AssertionGraphRefInput> {
    let value = value
        .cast::<PyDict>()
    ...
    let graph_kind = match field("graph_kind")?.extract::<String>()?.as_str() {
        "node" => gf_api::GraphObjectKind::Node,
        "edge" => gf_api::GraphObjectKind::Edge,
        _ => {
            return Err(PyTypeError::new_err("graph_kind must be 'node' or 'edge'"));
        }
    };
    let role = match field("role")?.extract::<String>()?.as_str() {
        "subject" => gf_api::AssertionGraphRole::Subject,
        "object" => gf_api::AssertionGraphRole::Object,
        "context" => gf_api::AssertionGraphRole::Context,
        _ => {
            return Err(PyTypeError::new_err(
    ...
    Ok(gf_api::AssertionGraphRefInput {
        graph_uuid,
        graph_kind,
        role,
        ordinal,
...
fn py_evidence_input(py: Python<'_>, value: &Bound<'_, PyAny>) -> PyResult<gf_api::EvidenceInput> {
    let value = value
        .cast::<PyDict>()
...
pub struct PyResolvedBeliefProjection {
    inner: Arc<gf_api::ResolvedBeliefProjection>,
}
...
impl PyResolvedBeliefProjection {
    ...
    fn prepare_paths_invocation(
        ...
        let terminal_uuids = parse_terminal_uuids(&terminal_uuids.unwrap_or_default())
            .map_err(|error| to_pyerr(py, &error))?;
        let options = gf_api::PathsOptions {
            by: by.parse().map_err(|error| to_pyerr(py, &error))?,
            via: via.map(str::to_owned),
            directed,
            k,
    ...
    fn prepare_analyze_invocation(
        ...
        let options = gf_api::AnalyzeOptions {
            by: by.parse().map_err(|error| to_pyerr(py, &error))?,
            via: via.map(str::to_owned),
            directed,
            weight: weight.map(str::to_owned),
    ...
    fn prepare_similar_invocation(
        ...
        let options = gf_api::SimilarOptions {
            by: by.parse().map_err(|error| to_pyerr(py, &error))?,
            k,
            vector_property: vector_property.map(str::to_owned),
            via: via.map(str::to_owned),
...
fn parse_algorithm_id(value: &str) -> Result<gf_api::Algorithm, GfError> {
    let (verb, name) = value
        .split_once('.')
    ...
    gf_api::Algorithm::parse(verb, name)
...
pub struct PyCancellationToken {
    inner: gf_api::CancellationToken,
}
...
impl PyCancellationToken {
    ...
    fn new() -> Self {
        Self {
            inner: gf_api::CancellationToken::new(),
        }
    }
...
impl GraphForge {
    ...
    fn checkpoint(
        ...
        self.ensure_open()?;
        let request = gf_api::CheckpointRequest {
            name,
            description,
    ...
    fn list_checkpoints(
        ...
        let result = py
            .detach(|| {
                self.inner.list_checkpoints(gf_api::ListCheckpointsRequest {
    ...
    fn delete_checkpoint(
        ...
        self.ensure_open()?;
        let request = gf_api::DeleteCheckpointRequest {
            name,
            idempotency_key: py_operation_id(idempotency_key)
                .map_err(|error| to_pyerr(py, &error))?,
            actor_uuid: actor_uuid
    ...
    fn revert_to_checkpoint(
        ...
        self.ensure_open()?;
        let request = gf_api::RevertCheckpointRequest {
            name,
            reason,
            idempotency_key: py_operation_id(idempotency_key)
                .map_err(|error| to_pyerr(py, &error))?,
    ...
    fn diff_checkpoints(
        ...
        let selector = |name: Option<String>| {
            name.map_or(
                ...
                gf_api::CheckpointSelector::Named,
            )
        };
        let scope = match scope {
            "summary" => gf_api::CheckpointDiffScope::Summary,
            "graph" => gf_api::CheckpointDiffScope::Graph,
            "ontology" => gf_api::CheckpointDiffScope::Ontology,
            "configuration" => gf_api::CheckpointDiffScope::Configuration,
            "capabilities" => gf_api::CheckpointDiffScope::Capabilities,
            "provenance" => gf_api::CheckpointDiffScope::Provenance,
            "knowledge" => gf_api::CheckpointDiffScope::Knowledge,
            "epistemic" => gf_api::CheckpointDiffScope::Epistemic,
            "all" => gf_api::CheckpointDiffScope::All,
            _ => {
                return Err(to_pyerr(
                    py,
        ...
        let detail = match detail {
            "summary" => gf_api::CheckpointDiffDetail::Summary,
            "records" => gf_api::CheckpointDiffDetail::Records,
            _ => {
                return Err(to_pyerr(
                    py,
        ...
        let result = py
            .detach(|| {
                self.inner.diff_checkpoints(gf_api::DiffCheckpointsRequest {
    ...
    fn enable_capability(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .enable_capability(gf_api::EnableCapabilityRequest {
    ...
    fn list_provenance_history(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .list_provenance_history(gf_api::ProvenanceHistoryRequest {
                        subject_uuid,
                        operation_uuid,
                        page: gf_api::PageRequest {
                            limit,
    ...
    fn create_assertion(
        ...
        let result = py
            .detach(|| {
                self.inner.create_assertion(gf_api::CreateAssertionRequest {
                    context: WriteContext {
                        operation_uuid,
                        actor_uuid,
                    },
        ...
        let result = py
            .detach(|| {
                self.inner.create_assertion_with_evidence(
                    gf_api::CreateAssertionWithEvidenceRequest {
    ...
    fn create_assertion_with_status(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .create_assertion_with_status(gf_api::CreateAssertionWithStatusRequest {
                        assertion: gf_api::CreateAssertionRequest {
                            context: WriteContext {
                                operation_uuid,
                                actor_uuid,
    ...
    fn list_assertions(
        ...
        let result = py
            .detach(|| {
                self.inner.list_assertions(gf_api::ListAssertionsRequest {
                    graph_uuid,
                    page: gf_api::PageRequest {
                        limit,
                        after,
    ...
    fn assess_confidence(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .assess_confidence(gf_api::AssessConfidenceRequest {
                        context: WriteContext {
                            operation_uuid,
                            actor_uuid,
                        },
    ...
    fn list_confidence_assessments(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .list_confidence_assessments(gf_api::ListConfidenceAssessmentsRequest {
                        assertion_uuid,
                        page: gf_api::PageRequest {
                            limit,
                            after,
    ...
    fn attach_evidence(
        ...
        let result = py
            .detach(|| {
                self.inner.attach_evidence(gf_api::AttachEvidenceRequest {
                    context: WriteContext {
                        operation_uuid,
                        actor_uuid,
                    },
    ...
    fn list_evidence_links(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .list_evidence_links(gf_api::ListEvidenceLinksRequest {
                        assertion_uuid,
                        source_uuid,
                        page: gf_api::PageRequest {
                            limit,
    ...
    fn record_reasoning(
        ...
        let result = py
            .detach(|| {
                self.inner.record_reasoning(gf_api::RecordReasoningRequest {
                    context: WriteContext {
                        operation_uuid,
                        actor_uuid,
                    },
    ...
    fn list_reasoning(
        ...
        let result = py
            .detach(|| {
                self.inner.list_reasoning(gf_api::ListReasoningRequest {
                    assertion_uuid,
                    page: gf_api::PageRequest {
                        limit,
                        after,
    ...
    fn record_assertion_status(
        ...
        let actor_uuid = actor_uuid
            ...
            .map_err(|error| to_pyerr(py, &error))?
            .map(|id| id.0);
        let request = gf_api::RecordAssertionStatusRequest {
            context: WriteContext {
                operation_uuid,
                actor_uuid,
            },
    ...
    fn list_assertion_status(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .list_assertion_status(gf_api::ListAssertionStatusRequest {
                        assertion_uuid,
                        page: gf_api::PageRequest {
                            limit,
                            after,
    ...
    fn supersede_assertion(
        ...
        let actor_uuid = actor_uuid
            ...
            .map_err(|error| to_pyerr(py, &error))?
            .map(|id| id.0);
        let request = gf_api::SupersedeAssertionRequest {
            context: WriteContext {
                operation_uuid,
                actor_uuid,
            },
    ...
    fn list_assertion_supersessions(
        ...
        let after = after
            ...
            .map_err(|error| to_pyerr(py, &error))?;
        let cancellation = cancellation.map(|token| token.inner.clone());
        let request = gf_api::ListAssertionSupersessionsRequest {
    ...
    fn create_hypothesis_group(
        ...
        let request = gf_api::CreateHypothesisGroupRequest {
            context: WriteContext {
    ...
    fn record_hypothesis_membership(
        ...
        let request = gf_api::RecordHypothesisMembershipRequest {
            context: WriteContext {
                operation_uuid: parse(operation_uuid)?,
                actor_uuid: actor_uuid.map(parse).transpose()?.map(|id| id.0),
            },
    ...
    fn record_hypothesis_selection(
        ...
        let parse =
            |value: &str| canonical_operation_id(value).map_err(|error| to_pyerr(py, &error));
        let request = gf_api::RecordHypothesisSelectionRequest {
            context: WriteContext {
                operation_uuid: parse(operation_uuid)?,
                actor_uuid: actor_uuid.map(parse).transpose()?.map(|id| id.0),
            },
    ...
    fn remove_hypothesis_member(
        ...
        let parse =
            |value: &str| canonical_operation_id(value).map_err(|error| to_pyerr(py, &error));
        let request = gf_api::RemoveHypothesisMemberRequest {
            context: WriteContext {
                operation_uuid: parse(operation_uuid)?,
                actor_uuid: actor_uuid.map(parse).transpose()?.map(|id| id.0),
            },
    ...
    fn list_hypothesis_groups(
        ...
        self.ensure_open()?;
        let request = gf_api::ListHypothesisGroupsRequest {
            question_key,
            page: gf_api::PageRequest {
                limit,
                after: after
    ...
    fn list_hypothesis_membership(
        ...
        let request = gf_api::ListHypothesisMembershipRequest {
    ...
    fn list_hypothesis_selection(
        ...
        let group_uuid = group_uuid
            ...
            .map_err(|error| to_pyerr(py, &error))?
            .map(|id| id.0);
        let request = gf_api::ListHypothesisSelectionRequest {
            group_uuid,
            page: gf_api::PageRequest {
                limit,
                after: after
    ...
    fn record_assertion_validity(
        ...
        let parse =
            |value: &str| canonical_operation_id(value).map_err(|error| to_pyerr(py, &error));
        let request = gf_api::RecordAssertionValidityRequest {
            context: WriteContext {
                operation_uuid: parse(operation_uuid)?,
                actor_uuid: actor_uuid.map(parse).transpose()?.map(|id| id.0),
            },
    ...
    fn list_assertion_validity(
        ...
        let result = py
            .detach(|| {
                self.inner
                    .list_assertion_validity(gf_api::ListAssertionValidityRequest {
                        assertion_uuid,
                        page: gf_api::PageRequest {
                            limit,
                            after,
    ...
    fn apply_valid_time(
        ...
        let result = py
            .detach(|| {
                self.inner.apply_valid_time(gf_api::ApplyValidTimeRequest {
    ...
    fn invoke_recorded(
        ...
        let recorded = py
            .detach(|| {
                self.inner
                    .invoke_recorded(gf_api::RecordedAlgorithmRequest {
                        context: WriteContext {
                            operation_uuid,
                            actor_uuid,
                        },
    ...
    fn resolve_belief_projection(
        ...
        py.detach(|| {
            self.inner
                .resolve_belief_projection(gf_api::ResolveBeliefProjectionRequest {
                    transaction_cutoff_micros: transaction_cutoff,
                    valid_time_micros: valid_time,
                    policy,
                })
        ...
        let result = py
            .detach(|| {
                self.inner.invoke_resolved_recorded(
                    &projection,
                    gf_api::ResolvedRecordedAlgorithmRequest {
    ...
    fn attach_resolved_run(
        ...
        let parse =
            |value: &str| canonical_operation_id(value).map_err(|error| to_pyerr(py, &error));
        let projection = Arc::clone(&projection.inner);
        let request = gf_api::AttachResolvedRunRequest {
            context: WriteContext {
                operation_uuid: parse(operation_uuid)?,
                actor_uuid: actor_uuid.map(parse).transpose()?.map(|id| id.0),
            },
    ...
    fn cluster(
        ...
        self.ensure_open()?;
        let opts = gf_api::ClusterOptions {
            by: by.parse().map_err(|error| to_pyerr(py, &error))?,
            vector_property: vector_property.map(str::to_owned),
            via: via.map(str::to_owned),
            directed,
    ...
    fn analyze(
        ...
        if matches!(
            ...
            let directed =
                directed.unwrap_or(!matches!(algorithm, gf_api::AnalyzeAlgorithm::GraphSage));
        ...
        let opts = gf_api::AnalyzeOptions {
            by: algorithm,
            via: via.map(str::to_owned),
    ...
    fn similar(
        ...
        self.ensure_open()?;
        let opts = gf_api::SimilarOptions {
            by: by.parse().map_err(|error| to_pyerr(py, &error))?,
            k,
            vector_property: vector_property.map(str::to_owned),
            via: via.map(str::to_owned),
...
fn parse_belief_projection_policy(
    ...
    supersession_branches: &str,
    hypotheses: &str,
) -> Result<gf_api::BeliefProjectionPolicyV1, GfError> {
    ...
    let statusless = match statusless {
        "reject" => gf_api::StatuslessPolicyV1::Reject,
        "exclude" => gf_api::StatuslessPolicyV1::Exclude,
        "include" => gf_api::StatuslessPolicyV1::Include,
        _ => {
    ...
    let supersession_branches = match supersession_branches {
        "reject" => gf_api::SupersessionBranchPolicyV1::Reject,
        "include_all_leaves" => gf_api::SupersessionBranchPolicyV1::IncludeAllLeaves,
        _ => {
            return Err(GfError::Validation(
    ...
    Ok(gf_api::BeliefProjectionPolicyV1 {
        included_statuses,
        statusless,
        supersession_branches,
        hypotheses,
...
fn resolved_recorded_result_to_python(
    py: Python<'_>,
    value: gf_api::ResolvedRecordedAlgorithmResult,
    ...
    match value.attachment {
        gf_api::ResolvedAttachmentOutcome::Attached(attachment) => {
        ...
        gf_api::ResolvedAttachmentOutcome::Failed {
            attachment_uuid,
            run_uuid: _,
            error_code,
        } => Ok(PyResolvedRecordedAlgorithmResult {
...
struct WriterHoldProbeState {
    held: Option<gf_api::concurrency_test_support::HeldWriter>,
}
...
fn _test_acquire_writer_hold(py: Python<'_>, path: &str) -> PyResult<()> {
    ...
    let held = py
        .detach(|| gf_api::concurrency_test_support::hold_writer(&owned))
        .map_err(|error| to_pyerr(py, &error))?;
