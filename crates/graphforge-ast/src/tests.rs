//! Unit tests for graphforge-ast: Token, ParseError, and AST node types.
...
// ---------------------------------------------------------------------------
// graphforge-cypher stub
// ---------------------------------------------------------------------------

#[test]
fn cypher_parse_stub_returns_not_implemented() {
    use crate::parse_error::ParseErrorKind;
    // Import parse from the sibling crate via re-export in graphforge-cypher's tests,
    // but since we're inside graphforge-ast we just verify the error types directly.
    let e = ParseError::new(
        ParseErrorKind::UnexpectedEof {
