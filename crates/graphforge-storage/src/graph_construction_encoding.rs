...
pub struct GraphConstructionEncodingEvidence {
    ...
    /// Exact bytes of the adjacency CSR artifacts published with the
    /// canonical inventory (#1388). Also counted in `output_write_bytes`; the
    /// bounded builder's individual write and fsync calls are not attributed.
    #[serde(default)]
    pub adjacency_write_bytes: u64,
    /// Projected edge rows the adjacency builder streamed from the encoded
    /// edge tables.
    #[serde(default)]
    pub adjacency_source_rows: u64,
    /// Sorted spill runs the adjacency builder wrote and merged.
    #[serde(default)]
    pub adjacency_spill_runs: u64,
    /// Peak bytes charged to the adjacency spill session.
    #[serde(default)]
    pub adjacency_spill_peak_bytes: u64,
    /// CSR shards published across every relation/direction pair.
    #[serde(default)]
    pub adjacency_csr_shards: u64,
}
...
fn encode_adjacency(
    ...
    for relative in relative_paths {
        ...
        add_evidence_counter(
            &mut evidence.adjacency_write_bytes,
            artifact.bytes,
    ...
    evidence.adjacency_source_rows = metrics.source_rows;
    evidence.adjacency_spill_runs = metrics.spill_runs;
    evidence.adjacency_spill_peak_bytes = metrics.spill_bytes;
    evidence.adjacency_csr_shards = metrics.csr_shards;
    Ok(())
}
