...
mod lifecycle_budget {
    ...
    fn range_partition_work_is_linear_across_production_chunk_counts() {
        ...
        for records in [31_u64, 32, 33, 272, 1023, 1024, 1025, 1088] {
            let root = TempDir::new().unwrap();
            crate::open_or_initialize_project(root.path()).unwrap();
            ...
            assert_eq!(evidence.merge_read_records, records * 2, "records={records}");
            assert_eq!(
            ...
            assert_eq!(evidence.partition_rows, records, "records={records}");
            assert_eq!(evidence.merge_passes, 0, "records={records}");
            assert_eq!(evidence.partition_outputs, 1, "records={records}");
            assert!(
                evidence.peak_partition_records * u64::try_from(plan.partitions()).unwrap()
                    <= records * 4,
                "records={records} peak={}",
