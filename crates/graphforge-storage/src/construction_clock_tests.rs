...
fn cross_process_shape_digest_helper() {
    ...
    if std::env::var("GF_TEST_SHAPE_PROPERTIES").as_deref() == Ok("1") {
        for (index, window) in nodes.chunks(128).enumerate() {
            ...
            fields.push(Arc::new(arrow::datatypes::Field::new("score", arrow::datatypes::DataType::Int64, false)));
            let mut columns = bare.columns().to_vec();
            columns.push(Arc::new(arrow::array::Int64Array::from(vec![7; window.len()])));
            let rows = RecordBatch::try_new(Arc::new(arrow::datatypes::Schema::new(fields)), columns).unwrap();
            session.append(ConstructionChunkKind::Node, &format!("nodes-{index}"), &rows).unwrap();
        }
        for (index, window) in edges.chunks(128).enumerate() {
            session.append(ConstructionChunkKind::Edge, &format!("edges-{index}"), &edge_rows(index * 128, window, &nodes)).unwrap();
        }
    } else {
    ...
    std::fs::write(output, serde_json::to_vec(&serde_json::json!({
...
fn identical_input_shaped_outputs_match_across_processes() {
    ...
    for properties in ["0", "1"] {
        ...
        for index in 0..2 {
            let output = outputs.path().join(format!("shape-{properties}-{index}.json"));
            let child = std::process::Command::new(std::env::current_exe().unwrap())
                .args(["--exact", "graph_construction::tests::determinism::cross_process_shape_digest_helper", "--nocapture"])
            ...
            let map: std::collections::BTreeMap<String, String> = serde_json::from_value(evidence["digests"].clone()).unwrap();
            assert!(map.contains_key(SHAPED_RUNTIME_CATALOG));
            assert!(map.len() >= 5);
            assert_eq!(map.keys().any(|name| name.starts_with("shaped-rows-")), properties == "1");
            maps.push(map);
            clocks.push(evidence["session_now_micros"].as_i64().unwrap());
        }
        assert_ne!(clocks[0], clocks[1], "independent processes must exercise different session clocks");
        println!("CROSS_PROCESS_SHAPE_DIGESTS={}", serde_json::json!({"properties":properties,"clocks":clocks,"digests":maps}));
