#!/usr/bin/env python3
"""Validate and execute the bounded correction-churn release workflow."""
...
def validate_evidence(record: dict[str, object], sha: str) -> None:
    ...
    if record.get("history", {}).get("append_only") is not True:
        raise ValueError("append-only correction history was not proved")
    if record.get("idempotency", {}).get("conflict_code") != "GF_IDEMPOTENCY_CONFLICT":
...
def main() -> None:
    ...
    output = (
        args.output or ROOT / "target/release-workflows/correction-churn/evidence.json"
    ).resolve()
    ...
    env = os.environ.copy()
    env["GF_CORRECTION_CHURN_SHA"] = actual
    env["GF_CORRECTION_CHURN_EVIDENCE"] = str(rust_evidence)
    env.setdefault(
    ...
    with tempfile.TemporaryDirectory(prefix="gf-correction-churn-") as temporary:
        temp = Path(temporary)
        ...
        if python_record["commit_sha"] != actual or python_record["reopen_equal"] is not True:
            raise SystemExit("Python correction/reopen evidence is stale")
