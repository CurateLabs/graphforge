#!/usr/bin/env python3
"""Validate and execute the bounded #2468 finance workflow."""
...
def validate_bundle() -> dict[str, object]:
    ...
    if missing:
        raise SystemExit(f"incomplete finance-fraud bundle: {missing}")
    scenario = json.loads((BUNDLE / "scenario.yaml").read_text())
    generator = json.loads((BUNDLE / "generator.yaml").read_text())
    if scenario["id"] != "finance-fraud" or scenario["owning_issue"] != 2468:
    ...
    expected_fingerprint = "sha256:" + digest(BUNDLE / "generator.yaml")
    if scenario["generator"]["fixture_fingerprint"] != expected_fingerprint:
        raise SystemExit("generator fixture fingerprint is stale")
    feature_ids = re.findall(r"\[(FF-\d{2})\]", (BUNDLE / "workflow.feature").read_text())
    manifest_ids = [step["id"] for step in scenario["steps"]]
    if feature_ids != manifest_ids or len(feature_ids) != len(set(feature_ids)):
        raise SystemExit("feature and manifest steps must map one-to-one in order")
    if not all(step.get("implementation") and step.get("evidence") for step in scenario["steps"]):
        raise SystemExit("every step must name its executable implementation and evidence")
...
def validate_provenance(
    ...
    *,
    ...
    if record.get("commit_sha") != commit_sha:
        raise ValueError("binding commit SHA mismatch")
    if record.get("wheel_sha256") != wheel_sha256:
        raise ValueError("binding wheel SHA mismatch")
...
def prove_mutation_sensitivity(record: dict[str, object], expected: dict[str, object]) -> None:
    for field, value in {
        ...
        changed[field] = value
        try:
            validate_provenance(changed, **expected)
        except ValueError:
            continue
...
def main() -> None:
    ...
    if not re.fullmatch(r"[0-9a-f]{40}", sha):
        raise SystemExit("checked-out SHA is malformed")
    ...
    rust_path = evidence_dir / "finance-fraud-rust.json"
    python_path = evidence_dir / "finance-fraud-python.json"
    aggregate_path = evidence_dir / "finance-fraud.json"
    env = os.environ.copy()
    ...
    with tempfile.TemporaryDirectory(prefix="gf-finance-python-") as temporary:
        ...
        record = json.loads(python_path.read_text())
        record.update(
            {
                "commit_sha": sha,
        ...
        python_path.write_text(json.dumps(record, indent=2, sort_keys=True) + "\n")
        expected = {
            "commit_sha": sha,
            "wheel_sha256": wheel_sha256,
    ...
    aggregate = {
        ...
        "rust_evidence_sha256": digest(rust_path),
        "python_evidence_sha256": digest(python_path),
        "outcome": rust,
        "binding": {
