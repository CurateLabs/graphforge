...
LICENSE_CHECK = Path(__file__).resolve().parents[2] / "scripts" / "license_check.py"
LP_SPEC = importlib.util.spec_from_file_location("license_check", LICENSE_CHECK)
assert LP_SPEC and LP_SPEC.loader
license_check = importlib.util.module_from_spec(LP_SPEC)
LP_SPEC.loader.exec_module(license_check)


def test_python_package_declares_apache_license_files() -> None:
    assert verify_package_licenses.verify_python_package() == []


def test_agent_skills_manifest_expects_license_and_notice() -> None:
    path = license_check.ROOT / "packages" / "agent-skills" / "package.json"
    assert path in license_check.MANIFEST_EXPECTATIONS
    snippets = license_check.MANIFEST_EXPECTATIONS[path]
