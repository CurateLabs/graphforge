...
/// Determine the outer orchestrator's execution signal for a compatible read by
/// probing the output directory. A resource-limit sidecar and a harness-error
/// sidecar are kept distinct from a produced-output file so the three failure
/// lanes are never conflated.
fn execution_signal(output_dir: &str, dataset_id: &str, code: &str) -> Option<ExecutionSignal> {
    let base = PathBuf::from(output_dir);
    let limit = base.join(format!("{dataset_id}-{code}.limit"));
    if limit.is_file() {
        let detail = fs::read_to_string(&limit).unwrap_or_default();
        return Some(ExecutionSignal::ResourceLimit(trim_detail(&detail)));
    }
    let harness = base.join(format!("{dataset_id}-{code}.harness"));
    if harness.is_file() {
        let detail = fs::read_to_string(&harness).unwrap_or_default();
        return Some(ExecutionSignal::HarnessError(trim_detail(&detail)));
    }
    let output = base.join(format!("{dataset_id}-{code}.out"));
    if output.is_file() {
        return match load_result_rows(&output) {
            Ok(rows) => Some(ExecutionSignal::Produced(rows)),
            Err(error) => Some(ExecutionSignal::HarnessError(error.to_string())),
        };
    }
    None
}
...
fn run_suite(
    ...
    if declared != Operation::ALL.to_vec() {
        return Err(format!(
            "suite requires exactly the {} modeled FinBench Transaction operations, found {}",
            Operation::ALL.len(),
            declared.len()
        ));
    }
    ...
    for job in &jobs {
        ...
        let (reference, signal) = if is_read {
            let reference_path = PathBuf::from(reference_dir)
                .join(format!("{dataset_id}-{}.ref", job.operation.code()));
            let reference = if reference_path.is_file() {
                Some(load_result_rows(&reference_path).map_err(|error| error.to_string())?)
            } else {
                None
            };
            let signal = execution_signal(output_dir, &dataset_id, job.operation.code());
            (reference, signal)
        } else {
            (None, None)
        };
        outcomes.push(run_job(job, reference.as_ref(), signal));
    }
    ...
    // Any real failure lane (correctness, resource, harness) fails the run.
