...
Counters via `graphforge_storage::io_stats`: `*_filtered_rows` = rows materialized;
...
_I/O gate: `cargo test -p graphforge-exec --release --test bench_traversal_scaling -- --ignored --nocapture --test-threads=1`. Wall-clock evidence: `make bench-traversal` (`cargo bench -p graphforge-exec --bench traversal_scaling`). Timing is hardware-dependent; `*_scanned_rows` assertions are the correctness gate._
