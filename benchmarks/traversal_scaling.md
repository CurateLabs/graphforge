...
_Generated by `make bench-traversal` (`cargo test -p graphforge-exec --release --test bench_traversal_scaling -- --ignored --nocapture --test-threads=1`). Wall-clock is hardware-dependent and illustrative; the `*_scanned_rows` assertions are the gate._
