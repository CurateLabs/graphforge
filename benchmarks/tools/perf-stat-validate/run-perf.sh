...
wait_quiet() {
  local n=0
  # also wait for our own workloads from any previous pass/run to be gone
  while pgrep -x perf >/dev/null || pgrep -x sort >/dev/null || pgrep -x openssl >/dev/null || pgrep -x gf >/dev/null; do
    echo "$(date -u +%FT%TZ) $1: previous workload still running, waiting"; sleep 2
  done
...
pstat() { # name pass events-or-metrics -- cmd...
  ...
  sudo -n perf stat $flag "$spec" -o "$OUT/$name.pass$pass.txt" -- sudo -n -u "$USER" env HOME="$HOME" PATH="$PATH" TMPDIR="$TMPDIR" "$@" > "$OUT/$name.pass$pass.stdout" 2>&1 || echo "exit $? for $name-$pass" | tee -a "$OUT/runs.log"
}
...
sudo -n rm -rf "$WS/s$SCALE"; mkdir -p "$WS/s$SCALE"
...
prep() { # fresh project, session begun, parquet registered — not measured
  sudo -n rm -rf "$WS/s$SCALE/source"
