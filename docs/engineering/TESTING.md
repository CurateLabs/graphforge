...
| D2 — bounded, identity-preserving state | #1576: `crates/graphforge-api/tests/slices.rs::in_memory_decision_input_composes_bounded_slice_and_explicit_query_without_retention` proves a memory-only Slice plus explicit-field Arrow query preserves canonical UUIDs, omits private properties and creates no Version. `selected_branch_decision_context_reads_exact_version_after_parent_change_reopen_and_cleanup` composes frozen Slice → Branch → exact-Version query, checks stable Slice identity across unrelated parent changes, and reopens after cleanup. `selectors_page_bounds_and_cancellation_use_real_execution`, `source_artifact_evidence_closure_stays_separate_from_membership`, and `selected_history_never_falls_back_to_current_or_retains_its_ancestor` cover cancellation/oversize, evidence dependencies and explicit expiry/unavailability. |
