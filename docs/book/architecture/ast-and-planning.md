...
| MERGE | Partial graph upsert: standalone new-node and referenced-endpoint relationship forms with write-path locking; multi-node construction and row-conditional map actions are rejected (`write_driver/create_merge.rs`) |
