...
ledgers. Before this decision, `graphforge-storage` owned obsolete domain schemas and
...
```text
graph/compiler/runtime crates ─> graphforge-core
graphforge-provenance ─────────────────> graphforge-core
graphforge-knowledge ──────────────────> graphforge-core
...
graphforge-bindings-node├─> graphforge-api
graphforge-cli          ┘
```

The domain crates may also depend on third-party Arrow, serialization, hashing,
and error libraries. Among workspace crates they depend on `graphforge-core` only.
`graphforge-provenance` and `graphforge-knowledge` do not depend on each other. Cross-domain
references are UUIDs from `graphforge-core`; `graphforge-api` validates and commits them.

The following directions are mechanically forbidden:

- `graphforge-storage`, `graphforge-exec`, `graphforge-rel`, `graphforge-plan`, `graphforge-search`, `graphforge-ir`,
  `graphforge-cypher`, `graphforge-ast`, `graphforge-ontology`, and `graphforge-io` to `graphforge-provenance` or
  `graphforge-knowledge`;
- `graphforge-provenance` or `graphforge-knowledge` to `graphforge-storage` or any graph/compiler
  crate;
...
and fails on these directions. It also applies when `graphforge-knowledge` is first
...
|---|---|---|
| `graphforge-core` | shared UUID aliases/newtypes, stable error envelope/code carrier, capability/version descriptor primitives, canonical byte/fingerprint primitives | domain records, domain enums, Arrow schemas, storage paths, orchestration |
| `graphforge-provenance` | provenance event and lineage records, closed kind/role registries, validation, deterministic ordering, canonical record bytes/fingerprints, authoritative Arrow schemas, domain errors | graph mutation execution, files/generations, assertions/confidence/evidence, cross-domain commits |
| `graphforge-knowledge` | all knowledge assertion/confidence/evidence/run records and all epistemic status/amendment/reasoning/supersession/hypothesis/selection/valid-time records; their registries, validation, ordering, canonical bytes/fingerprints, Arrow schemas, domain errors | graph execution, files/generations, provenance semantics, public orchestration |
| `graphforge-storage` | serialized project capability manifest and committed-generation record, transaction journal/lock, resolved project generations, generic typed participant descriptors, staging, checksums, fsync/publication, recovery, retention, Parquet/IPC mechanics, path containment | provenance/knowledge records or enums, epistemic policy, cross-domain reference meaning |
| `graphforge-api` | public Rust facade and neutral invocation descriptors, request validation, neutral graph-mutation receipt conversion, cross-domain reference validation, composite atomic-write participant assembly, capability-specific opens, pagination/cancellation, generated cross-domain schema inventory | duplicate domain schemas, binding-specific semantics |
| `graphforge-exec` | neutral graph mutation/inference receipts containing operation semantics and affected UUIDs | domain record construction; opening provenance/knowledge tables |
| Python/Node/CLI | thin projection of `graphforge-api`, Arrow/IPC transport, lossless error mapping | record/schema reimplementation, storage access, policy |

### Table ownership

Every table has exactly one semantic/schema owner. `graphforge-storage` may persist a
...
|---|---|---|---|
| provenance | `provenance/events` | `graphforge-provenance` | knowledge-layer |
| provenance | `provenance/lineage` | `graphforge-provenance` | knowledge-layer |
| project container | capability manifest, committed-generation record, participant descriptors, transaction journal | `graphforge-storage` (using version primitives from `graphforge-core`) | knowledge-layer |
| immutable knowledge | `knowledge/assertions` | `graphforge-knowledge` | knowledge-layer |
| immutable knowledge | `knowledge/assertion_graph_refs` | `graphforge-knowledge` | knowledge-layer |
| immutable knowledge | `knowledge/confidence_assessments` | `graphforge-knowledge` | knowledge-layer |
| immutable knowledge | `knowledge/evidence_links` | `graphforge-knowledge` | knowledge-layer |
| immutable knowledge | `knowledge/algorithm_runs` and `knowledge/algorithm_run_events` | `graphforge-knowledge` | knowledge-layer |
| epistemic | `knowledge/assertion_status_events` | `graphforge-knowledge` | epistemic |
| epistemic | `knowledge/assertion_amendments` | `graphforge-knowledge` | epistemic |
| epistemic | `knowledge/reasoning_events` | `graphforge-knowledge` | epistemic |
| epistemic | `knowledge/supersession_events` | `graphforge-knowledge` | epistemic |
| epistemic | `knowledge/hypothesis_membership_events` | `graphforge-knowledge` | epistemic |
| epistemic | `knowledge/hypothesis_selection_events` | `graphforge-knowledge` | epistemic |
| epistemic | optional `knowledge/valid_time_events` | `graphforge-knowledge` | epistemic |
...
There is no second schema definition in `graphforge-storage`, `graphforge-api`, bindings,
documentation, or tests. Those consumers obtain schemas or generated fixtures
from the owning registry.

`graphforge-api` assembles the two registries plus graph/workbench registries into the
...
2. **Composite pre-publication validation:** `graphforge-api` builds UUID indexes over
...
`graphforge-storage` verifies participant bytes/checksums and calls opaque validation
...
- `graphforge-provenance`: provenance schema/version/record/reference errors;
- `graphforge-knowledge`: knowledge/epistemic schema/version/record/reference errors;
- `graphforge-storage`: generation/participant/checksum/path/durability errors;
- `graphforge-api`: request, idempotency, cross-domain, and composite-transaction
  errors.
...
- `graphforge-storage` can atomically publish new participant kinds without changing
  for each domain.
- Cross-domain writes require deliberate `graphforge-api` orchestration and a
...
| Alternative | Reason |
|---|---|
| Keep knowledge schemas in `graphforge-storage` | Makes generic I/O own epistemic meaning and invites graph-open coupling. |
| Put all knowledge/epistemic records in `graphforge-provenance` | Conflates lineage with assertions, evidence, confidence, runs, and evolving belief. |
| Let `graphforge-knowledge` depend on `graphforge-provenance` | Turns a UUID reference into compile-time schema coupling and makes epistemic evolution capable of rewriting knowledge-layer provenance. |
