...
| Registry publication | v0.5.0 shipped PyPI + npm node only; CLI/skills/crates arrived later. | Treat “coordinated surface complete” as the post-release verify gate, not the first tag alone. Declined as a separate issue — already encoded by release-certification corrective releases. |
