...
8. `graphforge-portable-oci`
9. `graphforge-provenance`
10. `graphforge-value`
11. `graphforge-cypher`
12. `graphforge-ontology`
13. `graphforge-ir`
14. `graphforge-plan`
15. `graphforge-storage`
16. `graphforge-rel`
17. `graphforge-search`
18. `graphforge-exec`
19. `graphforge-api`
20. `graphforge-cli`
...
re-observes all three registries and produces one stable 29-node summary. Job
...
still emits all 29 node identities as `indeterminate` and identifies the
candidate blocker. The workflow is green only when all 29 nodes are publicly
...
- decide any registry-specific yank/deprecation if reconciliation finds a
  conflict; and
- close the human release tracker only after the 29-node summary is complete.
