...
GraphForge v0.5.2 is one 25-node release: 16 crates.io crates, one PyPI
...
1. `graphforge-core`
2. `graphforge-filesystem`
3. `graphforge-ast`
4. `graphforge-knowledge`
5. `graphforge-ontology`
6. `graphforge-provenance`
7. `graphforge-ir`
8. `graphforge-plan`
9. `graphforge-storage`
10. `graphforge-io`
11. `graphforge-rel`
12. `graphforge-search`
13. `graphforge-cypher`
14. `graphforge-exec`
15. `graphforge-api`
16. `graphforge-cli`
...
re-observes all three registries and produces one stable 25-node summary. Job
...
still emits all 25 node identities as `indeterminate` and identifies the
candidate blocker. The workflow is green only when all 25 nodes are publicly
...
- decide any registry-specific yank/deprecation if reconciliation finds a
  conflict; and
- close the human release tracker only after the 25-node summary is complete.
