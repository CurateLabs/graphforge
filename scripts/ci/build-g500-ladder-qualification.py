...
def rung(cert: dict) -> dict:
    ...
    rows.append(
        artifact(
            ...
            construction.get("storage_transient_peak_total_allocated_bytes", 0),
        )
    )
    rows.append(artifact("portable_package", storage["portable_package"], "exact_descriptor"))
    rows.append(
        artifact("clean_imported_project", storage["clean_import"], "clean_import_snapshot")
    )
    ...
    return {
        "id": f"S{cert['run']['scale']}",
        "scale": cert["run"]["scale"],
        "live_nodes": nodes,
        "live_edges": edges,
        "source_project_current_allocated_bytes": storage["source_project_current_allocated_bytes"],
        "workspace_current_allocated_bytes": storage["workspace_current_allocated_bytes"],
        "artifacts": rows,
        "phases": phases,
        "totals": totals,
        "ratios": {
            "canonical_node_bytes_per_live_node": {
                "numerator_bytes": by_name["canonical_node_topology"]["logical_bytes"],
                "denominator_count": nodes,
            },
            "canonical_edge_bytes_per_live_edge": {
                "numerator_bytes": by_name["canonical_edge_topology"]["logical_bytes"],
                "denominator_count": edges,
            },
            "authoritative_project_bytes_per_live_edge": {
                "numerator_bytes": storage["source_project_current_allocated_bytes"],
                "denominator_count": edges,
            },
            "full_lifecycle_peak_bytes_per_live_edge": {
                "numerator_bytes": totals["transient_peak_allocated_bytes"],
                "denominator_count": edges,
            },
        },
    }
...
def main() -> None:
    ...
    canonical_nodes = (
        by_category["canonical_node_topology"]["current_retained_bytes"] * target_nodes
        + high["live_nodes"]
        - 1
    ) // high["live_nodes"]
    canonical_edges = (
        by_category["canonical_edge_topology"]["current_retained_bytes"] * target_edges
        + high["live_edges"]
        - 1
    ) // high["live_edges"]
    ...
    decision = (
        "admit"
        if peak <= args.volume_bytes and headroom >= args.reserved_headroom_bytes
        else "refuse"
    )
    value = {
        ...
        "projection": {
            ...
            "rate": {"numerator_bytes": ratio_num, "denominator_count": ratio_den},
            "projected_canonical_node_bytes": canonical_nodes,
            "projected_canonical_edge_bytes": canonical_edges,
            "projected_lifecycle_peak_bytes": peak,
