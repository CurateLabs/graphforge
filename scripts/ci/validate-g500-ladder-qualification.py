...
REQUIRED_CATEGORIES = {
    ...
    "canonical_edge_topology",
    "properties",
    "uuid_surrogate_indexes",
    "adjacency_csr",
    "catalog_manifests",
...
def validate(evidence: dict[str, Any]) -> None:
    ...
    for rung in rungs:
        ...
        phase_totals = {
            f"phase_{field}": sum(phase[field] for phase in phases) for field in phase_fields
        }
        ...
        expected = {
            ...
            "full_lifecycle_peak_bytes_per_live_edge": {
                "numerator_bytes": transient_peak,
                "denominator_count": live,
            },
        }
    ...
    canonical_node_projected = ceil_ratio(
        latest_categories["canonical_node_topology"]["current_retained_bytes"] * S26_NODES,
        rungs[-1]["live_nodes"],
    )
    canonical_edge_projected = ceil_ratio(
        latest_categories["canonical_edge_topology"]["current_retained_bytes"] * S26_EDGES,
        rungs[-1]["live_edges"],
    )
    ...
    if (
        projected <= projection["volume_bytes"]
