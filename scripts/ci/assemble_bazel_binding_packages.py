...
def read_node_export_surface(package_root: Path) -> dict[str, str]:
    """Return the declared ``name -> kind`` napi export surface, sorted by name.

    The Bazel lane never runs ``napi build``, so it cannot read napi's generated
    ``index.js``/``index.d.ts``. The surface is declared in
    ``napi-export-surface.json`` beside ``package.json`` instead, and
    ``tests/export-surface.test.mjs`` fails closed when it drifts from the
    exports the built addon actually has.
    """
    manifest_path = package_root / EXPORT_SURFACE_FILENAME
    if not manifest_path.is_file():
        _die(f"missing napi export surface manifest: {manifest_path}")
    try:
        data = json.loads(manifest_path.read_text(encoding="utf-8"))
    except json.JSONDecodeError as error:
        _die(f"{manifest_path} is not valid JSON: {error}")
    exports = data.get("exports") if isinstance(data, dict) else None
    if not isinstance(exports, dict) or not exports:
        _die(f"{manifest_path} must declare a non-empty 'exports' object")
...
def read_node_package_identity(package_root: Path) -> tuple[str, str]:
    """Return ``(package name, napi binaryName)`` from the Node manifest (#1369).

    The synthesized loader names the optional platform packages
    ``<name>-<platformArchABI>`` and the sibling addon files
    ``<binaryName>.<platformArchABI>.node``, which is exactly how napi names
    what ``napi create-npm-dirs`` and ``napi artifacts`` produce.
    """
    manifest = json.loads((package_root / "package.json").read_text(encoding="utf-8"))
    name = manifest.get("name")
    if not isinstance(name, str) or not name:
        _die("could not parse name from package.json")
    napi = manifest.get("napi")
    binary_name = napi.get("binaryName") if isinstance(napi, dict) else None
    if not isinstance(binary_name, str) or not binary_name:
        _die("could not parse napi.binaryName from package.json")
    return name, binary_name


def _napi_host_tag_map() -> dict[str, str]:
    """Map ``${process.platform}-${process.arch}`` to napi's platformArchABI tag.

    napi names every published platform package and addon file after its
    ``platformArchABI`` (``linux-x64-gnu``, ``win32-x64-msvc``, ...), whose
    first two segments are exactly Node's ``process.platform`` and
    ``process.arch``. Deriving the table from the declared tag set keeps the
    synthesized loader and the assembled addon naming from drifting apart.
    """
    table: dict[str, str] = {}
    for tag in sorted(_NAPI_PLATFORM_TAGS):
        node_platform, node_arch = tag.split("-")[:2]
        key = f"{node_platform}-{node_arch}"
        if key in table:
            # Two tags on one host (a gnu/musl pair, say) would need a runtime
            # discriminator, the way napi's isMusl() probe is one.
            _die(f"ambiguous napi platform tags for {key}: {table[key]} and {tag}")
        table[key] = tag
    return table


def synthesize_node_index_js(
    addon_name: str,
    surface: dict[str, str],
    *,
    package_name: str,
    ...
    """CJS loader resolving the addon the way an install actually provides it.

    Resolution order mirrors napi's own generated loader (#1369): the sibling
    addon file first, then the optional scoped platform package. A published
    install only ever has the latter — ``package.json`` ``files`` excludes
    ``*.node`` and ``napi artifacts`` moves every addon into
    ``npm/<platformArchABI>/`` — so a loader carrying only the sibling require
    threw ``Cannot find module './graphforge.<tag>.node'`` on a clean consumer
    install. The Bazel Binding RC lane stages the addon beside ``index.js``
    instead, so both candidates have to stay.

    Node resolves ESM named imports of a CommonJS module with cjs-module-lexer,
    which only sees literal ``module.exports.<name> = ...`` assignments. An
    earlier loader assigned ``version`` literally and mirrored everything else
    through a ``for (const key of Object.keys(...))`` loop, which the lexer
    cannot see, so ``import { GraphForge } from '@curatelabs/graphforge'`` threw
    ``SyntaxError: Named export 'GraphForge' not found``. Every export is
    therefore emitted on its own literal line (#1360). The probe above those
    lines is invisible to the lexer, which reads only the export statements —
    exactly the shape of napi's own generated ``index.js``.
    """
    host_tags = _napi_host_tag_map()
    lines = [
        "// Generated by assemble_bazel_binding_packages.py (#7 / #720 / #1360 / #1369).",
        "// Loads the napi addon from wherever the install put it, in napi's own",
        "// resolution order (#1369):",
        "//   1. NAPI_RS_NATIVE_LIBRARY_PATH, when set;",
        f"//   2. ./{addon_name}, the addon the Bazel Binding RC lane stages beside",
        "//      this file;",
        "//   3. the running host's addon file beside this file;",
        f"//   4. the optional platform package {package_name}-<platformArchABI>,",
        "//      which is the only place a published install has the addon: the",
        "//      package.json `files` list excludes *.node, and `napi artifacts`",
        "//      moves every addon into npm/<platformArchABI>/.",
        "const hostTags = {",
    ]
    lines.extend(f"  '{key}': '{tag}'," for key, tag in sorted(host_tags.items()))
    lines.extend(
        [
            "};",
            "const platformArchABI = hostTags[`${process.platform}-${process.arch}`];",
            "",
            "const candidates = [];",
            "const addCandidate = (specifier) => {",
            "  if (specifier && !candidates.includes(specifier)) {",
            "    candidates.push(specifier);",
            "  }",
            "};",
            "addCandidate(process.env.NAPI_RS_NATIVE_LIBRARY_PATH);",
            f"addCandidate('./{addon_name}');",
            "if (platformArchABI) {",
            f"  addCandidate(`./{binary_name}.${{platformArchABI}}.node`);",
            f"  addCandidate(`{package_name}-${{platformArchABI}}`);",
            "}",
            "",
            ...
            "  }",
            "  throw new Error(",
            f"    `Cannot find the native binding for {package_name} on ` +",
            "      `${process.platform}-${process.arch}. npm has a bug related to ` +",
            "      `optional dependencies (https://github.com/npm/cli/issues/4828); ` +",
            "      `reinstall after removing package-lock.json and node_modules. ` +",
            "      `Tried:\\n  ` +",
            "      attempts.join(`\\n  `),",
            "  );",
            "}",
            ...
            "",
            "// Every named export below is assigned literally, one per line: Node",
            "// resolves ESM named imports of a CommonJS module with cjs-module-lexer,",
            "// which only detects literal `module.exports.<name> = ...` statements. A",
            "// dynamic mirror loop is invisible to it and breaks",
            f"// `import {{ GraphForge }} from '{package_name}'` (#1360).",
...
def synthesize_node_index_dts(surface: dict[str, str]) -> str:
    """Declare exactly the exports the assembled loader exposes at runtime."""
    lines = [
        "/* Generated by assemble_bazel_binding_packages.py (#7 / #720 / #1360). */",
...
def assemble_node(
    *,
    ...
    package_name, binary_name = read_node_package_identity(package_root)
    resolved_platform_tag = _napi_platform_tag(platform_tag)
    addon_name = f"{binary_name}.{resolved_platform_tag}.node"
    native_hash = _sha256(native)
    ...
    with tempfile.TemporaryDirectory(prefix="gf-bazel-node-pkg-") as tmp:
        ...
        # Always synthesize Bazel smoke loaders. Do not copy a local
        # napi-generated index.js (gitignored): the Bazel lane never runs
        # `napi build`, so the only loader it can trust is the one generated
        # from the declared export surface (#720). That loader probes the
        # optional platform packages the same way napi's does, because that is
        # where a published install has the addon (#1369).
        ...
        (staging / "index.d.ts").write_text(
            synthesize_node_index_dts(surface),
            encoding="utf-8",
        )
