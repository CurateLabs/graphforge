...
def synthesize_node_index_js(addon_name: str, surface: dict[str, str]) -> str:
    """CJS loader whose named exports resolve from ESM and CommonJS alike (#1360).

    Node resolves ESM named imports of a CommonJS module with cjs-module-lexer,
    which only sees literal ``module.exports.<name> = ...`` assignments. The
    previous loader assigned ``version`` literally and mirrored everything else
    ...
    therefore emitted on its own literal line.
    """
    lines = [
        "// Generated by assemble_bazel_binding_packages.py (#7 / #720 / #1360).",
        "// Loads the Bazel-built napi addon.",
        "//",
        ...
        "// `import { GraphForge } from '@curatelabs/graphforge'` (#1360).",
...
def assemble_node(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="gf-bazel-node-pkg-") as tmp:
        ...
        # napi-generated index.js (gitignored): it probes optional platform
        # packages and is not the Binding RC contract for Bazel natives (#720).
        (staging / "index.js").write_text(
            synthesize_node_index_js(addon_name, surface),
            encoding="utf-8",
        )
