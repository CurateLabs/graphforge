...
if grep -q "$sentinel" "$temporary_directory/plan.json"; then
...
sed \
  "s|\"secrets\":\\[\\]|\"secrets\":[{\"id\":\"service-token\",\"source\":\"secret_manager\",\"value\":\"$sentinel\"}]|" \
